...
- `MEDISTAR + Manuelle Dokumentuebergabe` ist als eigener V1-Kandidat vorhanden: AIS-Datei startet den Uebertragen-Dialog mit leerer Liste, der Anwender fuegt Dateien per Drag-&-Drop oder Dateiauswahl hinzu, und erst `Uebertragen` exportiert je Datei eigene `6302`-`6305`-Linkfelder. Ein Geraete-Importordner ist fuer diesen Modus nicht erforderlich.
- Die manuelle Dokumentuebergabe aktualisiert den offenen Dialog nur noch inkrementell und schreibt TextBox-Inhalte nicht mehr bei jedem Tastendruck zurueck. Polling stoert die Texteingabe dadurch deutlich weniger; die XDT-Fachlogik bleibt unveraendert.
...
- MEDISTAR + Dokumentanhang mit PDF/JPG/XML/MP4/MP3/WAV-Dateien praktisch pruefen, inklusive Ruhezeitmodus, manuellem Bestaetigungsmodus und sichtbarer `6304`-Beschreibung je Datei; Drag-&-Drop, Vorschaukachel-Galerie und OCR bleiben separate spaetere Themen.
...
8. MEDISTAR + Dokumentanhang im Live-Test mit mehreren Anhangdateien, Ruhezeit-/Bestaetigungsabschluss und pro-Datei-`6304`-Text pruefen.
