...
- `MEDISTAR + Manuelle Dokumentuebergabe` ist als eigener V1-Kandidat vorhanden: Erst eine AIS-Datei startet den Uebertragen-Dialog mit leerer Liste, der Anwender fuegt Dateien per Drag-&-Drop, Dateiauswahl oder Datei-Zwischenablage hinzu, und erst `Uebertragen` exportiert je Datei eigene `6302`-`6305`-Linkfelder. Ein Geraete-Importordner ist fuer diesen Modus nicht erforderlich.
- Die manuelle Dokumentuebergabe aktualisiert den offenen Dialog nur noch inkrementell, schreibt TextBox-Inhalte nicht mehr bei jedem Tastendruck zurueck und bleibt standardmaessig per `TOP` im Vordergrund. Polling stoert die Texteingabe dadurch deutlich weniger; die XDT-Fachlogik bleibt unveraendert.
