...
- Interne Web-V1: `XdtBox.LicenseWeb` als ASP.NET-Core-8-Razor-Pages-Projekt, aktuell `0.2.0-web-production-prep`, ohne Desktop-App-Versionsanhebung
...
- Die Web-V1 des Lizenzmanagers ist noch kein produktives Hosting. Sie bleibt intern/lokal, nutzt serverseitigen Login, Datenroot ausserhalb von `wwwroot`, Private-Key-Konfiguration nur serverseitig und wiederverwendet vorhandene PDF-/Backup-/Issuer-Bausteine. Die Produktivvorbereitung sperrt unsichere DataRoot-/Private-Key-Pfade, verlangt im Production-Modus einen Hash-Admin, bietet eine Produktivdiagnose und kann Einzelkunden anhand Kundennummer aus Backup oder Desktop-Datenroot uebernehmen. Eine spaetere Einbindung in die statische Website erfolgt nur per Link nach Sicherheits- und Hostingabnahme.
...
- Referenzdaten werden defensiv als Geraetequelle ausgewertet: XML-nahe und scriptbasiert belastbar ableitbare Geraete duerfen als isolierte BuiltIns mit Tests aufgenommen werden; echte Rohdaten bleiben fuer die Praxisabnahme erforderlich. Huvitz, TOMEY, Shin-Nippon, Reichert, Rodenstock-CX-800 und belastbare TOPCON-Varianten wurden inzwischen batchweise mit eigenen Profilketten beziehungsweise Parsern, synthetischen Fixtures und Templatepaket-Tests vorbereitet. Rodenstock Phoromat 2000 ist als inaktives Beta-BuiltIn mit eigenem seriellen Parser/Writer vorbereitet; echte RS232-Mitschnitte, Sendetest am Geraet und MEDISTAR-Abnahme bleiben offen. Moeller-Wedel Visutron bleibt Kandidat.
...
- Aus den Referenzdaten-Batches sind NIDEK ARK-510A, NIDEK ARK-560A und NIDEK LM-1800P als neue BuiltIns vorbereitet; TOPCON KR-800 wird ueber die vorhandene KR800S-Familie erkannt. Zusaetzlich sind TOPCON CL-300PDL, RM-800 und TRK-3 Omnia als eigene, testseitig abgesicherte Varianten der vorhandenen TOPCON-XML-Familien vorhanden. Reichert 7CR NCT, Reichert LensChek Plus und Rodenstock CX 800 sind als serielle Text-BuiltIns vorbereitet. Die vollstaendige Entscheidungstabelle steht in `docs/REFERENZPAKET_GERAETE_AUSWERTUNG.md`.
...
- RS232-Diagnose ist ueber den `XDT-Baukasten` und ueber `Schnittstellenprofile` erreichbar: zeitlich begrenzte Rohdaten-Mitschnitte mit Text-/Hexanzeige, DTR/RTS und Parameteruebernahme funktionieren ohne alten Profil-/Template-Tab. Der RT-3100-Produktivablauf nutzt den bestaetigten Pfad `RS senden, dann Writer ohne SD`; fruehere RT-Sendetestbuttons sind in der normalen Praxis-Geraetekachel ausgeblendet.
...
- Fuer die NIDEK-RT-Phoropterfamilie RT-2100 / RT-3100 / RT-5100 ist ein eigener serieller Parser-/Writer-/Kommunikationszweig vorhanden: PDF-basierte Presets, SH/SX/EB/ET/CR-Frameauswertung, RT->PC-Parsing fuer Final/Subjective, PC->RT-Sendeframes fuer LM-/AR-Daten und patientengetriggerter Produktivablauf. Die Ueberwachung oeffnet kein RT-Fenster beim Start; erst eine stabile AIS-Datei oeffnet den Auswahl-/Sendedialog. Beim RT-3100 sendet der normale Button automatisch `RS`, wartet kurz, sendet den Writer-Frame ohne SD-Pflicht und wartet anschliessend mit Patientenkontext auf die spaetere Rueckgabe bis `EOT` plus Stabilitaetswartezeit.
- Fuer Rodenstock Phoromat 2000 ist ein isolierter Beta-Zweig vorbereitet: RS232-Default `9600 8N1` ohne DTR/RTS/Handshake, Frameparser fuer Start-/Endemarkierungen, Parser fuer `FN`, `PD`, `WD` und optionalen VA-Platzhalter sowie ein PC->Geraet-Writer fuer die unterstuetzten Rueckgabewerte aus der AIS-Historie. Produktive Freigabe erfolgt erst nach echten RS232-Mitschnitten und Live-Abnahme.
...
- Produktive serielle Parser und ein dauerhafter RS232-Profilbetrieb muessen fuer weitere serielle Altgeraete anhand echter Rohdaten ergaenzt und praktisch validiert werden. Fuer NIDEK RT-3100 ist der vollstaendige Praxislauf bestaetigt: AIS-Patientendatei, Auswahlfenster, `RS senden, dann Writer ohne SD`, spaetere Rueckgabe, MEDISTAR-XDT-Ausgabe und terminales Schliessen ohne erneutes Oeffnen bis zur naechsten AIS-Datei. RT-2100 und RT-5100 nutzen denselben vorbereiteten Modus und brauchen je eine eigene Live-Abnahme.
...
- Der Tab `Schnittstellenprofile` ist fuer die naechsten Geraete-Batches bereinigt: Hersteller- und AIS-Filter stehen mit der begrenzten Schnittstellenprofil-Auswahl in einer kompakten Kopfzeile, aktive Schnittstellenprofile zaehlen immer als Geraeteanbindung, der fruehere Lizenzpflicht-Haken ist entfernt, und `AIS Ausgabe Info` zeigt eine reine AIS-Felduebersicht inklusive `8402`-Untersuchungsart-Hinweis und empfohlenen Defaultwerten. Die Defaultlogik priorisiert die Geraeteidentitaet vor optionalen Anhangsfaehigkeiten, damit Tonometer/Pachymeter wie NIDEK NT-1E `TONO` bleiben und nur reine Dokument-/AttachmentOnly-Profile `DOKU` erhalten. Reine Dokumentgeraete markieren `6302`, `6303` und `6305` als Karteikarten-relevant; Messgeraete mit optionalem Anhang behalten diese Felder optional. Das `6305`-Layout trennt Pfadfeld und Ordnerbuttons in eigenen Grid-Spalten. Neue BuiltIn-/Template-/Repair-Profile starten mit Ordnerbereinigung aktiv, Archivierung aus und Fehlerablage aktiv; UserDefined-Profile behalten bewusste Einstellungen.
...
- `MEDISTAR + NIDEK RT-6100` ist als bidirektionaler NIDEK-LAN-/MEM-200-Phoropter-Kandidat testseitig vorbereitet: PDF-Auswertung fuer Shared-Folder-/`DIRECT_RT_xx\TXT`-Ablauf, echte LM-7P-/ARK-1s-/RT-6100-XML-Fixtures, RT-6100-Inputwriter fuer `LM_Base`/`REF_Base`, Rueckgabeparser fuer `Best` unter `6228` und `Full` unter `6227`, BuiltIn-Geraeteprofil, Exportprofil, Schnittstellenprofil, Ausgabe-an-Geraet-Konfiguration, selektiver Templatepaket-Test und Protokollnotizen sind vorhanden. Das bereitgestellte OCR-XML ist malformed und dient nur als Diagnosefixture; positive Tests nutzen echte wohlgeformte NIDEK-Dateien plus synthetische Grenzfaelle.
- `MEDISTAR + NIDEK RT-2100/RT-3100/RT-5100 RS232` ist als serielle bidirektionale Phoropterfamilie vorbereitet und technisch produktiv angebunden: Die PDF-Auswertung liefert Type1-/Type2-Presets, gemeinsame SH/SX/EB/ET/CR-Steuerzeichen, RT->PC-Formate fuer Final/Subjective und PC->RT-Sendebloecke fuer LM-/AR-Daten. BuiltIn-Geraete-, Export- und Schnittstellenprofile, selektive Templatepaket-Tests, Baukasten-Geraeteausgabe-Vorschau und COM-Kommunikationsservice sind vorhanden. Der Workflow ist AIS-getriggert, sendet nur nach Anwenderklick, wartet bis `EOT` plus Stabilitaetswartezeit und erzeugt danach die MEDISTAR-Rueckgabe. Der RT-3100-Praxislauf ist mit `RS senden, dann Writer ohne SD`, Rueckgabe, XDT-Ausgabe und terminalem Fensterabschluss bestaetigt; RT-2100 und RT-5100 bleiben analog vorbereitet und live offen.
- `MEDISTAR + Rodenstock Phoromat 2000` ist als inaktive Beta-Profilkette vorbereitet: eigener RS232-Parser/Writer, `FN` nach `6228`, `WD` nach `6227`, `PD` in der Phoropterzeile und VA nur als Baukasten-Platzhalter. Die DeviceOutput-Vorschau zeigt den seriellen Sendeframe, schreibt aber im Baukasten keine produktiven Geraetedateien. Echte Livevalidierung und MEDISTAR-Abnahme fehlen noch.
...
- `Lizenz entfernen` entfernt nur die lokal importierte Lizenz und eine eventuelle Legacy-Lizenz; Profile, Kundendaten, Lizenzanfragen und Historien bleiben erhalten.
- Lizenzanforderungen bilden immer den aktuellen Gesamtzustand der Installation mit allen aktiven Geraeteanbindungen ab.
- Eine harte produktive Lizenzsperre ist noch nicht aktiv.
- Online-Lizenzierung ist weiterhin offen und fuer V1 nicht erforderlich.
...
- NIDEK NT530P: echte XML-Fixture, BuiltIn-Schnittstellenprofil, korrigierter mehrzeiliger `6205`-/`6220`-Export ohne EV-Zusatz, Mehrfachanhang-Linkfelder, Nachlauf-/Monitoring-Reset, Templatepaket-Kandidat und praktische Tono/Pachy-MEDISTAR-Abnahme sind vorhanden; JPG-/Mehrfachanhang-Sonderfaelle bleiben offen.
...
- Canon/ZEISS/Visionix Batch 8: Canon RK-F2, Canon TX-20P, ZEISS VISULENS 550, ZEISS VISUPLAN 500, ZEISS VISUREF 100, ZEISS IOLMaster 700, Visionix Retinomax 5, Visionix VX 120 und Visionix VX 650 sind aus statisch gepruefter Referenzlogik als BuiltIn-Geraete-, Export- und Schnittstellenprofile vorbereitet. `CanonZeissVisionixDeviceParser` trennt REF/Lensmeter nach `6228`, KM nach `6221`, Tonometrie nach `6205` und Pachymetrie/CCT nach `6220`; `ZeissIolMaster700DeviceParser` trennt VKT/AL nach `6227` und R1/R2-Keratometrie nach `6228`. OCT-Geraete bleiben ausgeschlossen.
...
- NIDEK RT6100: bidirektionaler LAN-/MEM-200-Phoropter-Kandidat mit RT-6100-Ophthalmology-Inputwriter, Rueckgabeparser, `6228`-Best-Export, `6227`-Full-Export, BuiltIn-Profilen und Templatepaket-Kandidat; praktische MEM-200-/Geraete-/MEDISTAR-Abnahme offen.
- Fertige, auslieferbare Geraete-Templatepakete als ZIP-Artefakte. Fuer ARK1S, AR360, LM7, NT530P, NIDEK Batch 6 XML-Varianten, RT-2100/RT-3100/RT-5100 RS232, CL300, Solos, KR800S, KR-1, TRK2P, CT1P, CT800A, CV5000, RT6100 und Dokumentanhang sind Referenz- beziehungsweise Kandidatendokumentation und reproduzierbare Export-/Import-Tests bereits vorhanden; dauerhafte ZIPs folgen erst nach `docs/TEMPLATEPAKET_RELEASE_REGEL.md`.
...
- NIDEK NT530P als validierten Tono/Pachy-Workflow halten und JPG-/Mehrfachanhang-Sonderfaelle mit sauberem Ordner-/Karten-Nachlauf separat abnehmen; NIDEK Batch 6 XML-Varianten mit echten Praxisrohdateien gegen die vorbereiteten BuiltIns pruefen; TOPCON CL300, TOPCON KR800S, TOPCON TRK2P und TOPCON CT1P als praktisch validierte TOPCON-Referenzkandidaten beibehalten, H/V-Prisma beziehungsweise SBJ-Funktionstestfaelle weiter beobachten; TOPCON Solos, TOPCON KR-1 und TOPCON CT800A praktisch in MEDISTAR abnehmen und echte Transmission-/PDF-, KM/KRT- beziehungsweise weitere Tonometrie-Faelle sammeln; Canon/ZEISS/Visionix Batch 8 inklusive IOLMaster 700 mit echten Praxisrohdateien gegen die synthetischen Fixtures validieren und OCT weiterhin ausklammern; TOPCON CV5000 und NIDEK RT6100 weiter bidirektional live pruefen; RT-2100/RT-5100 analog zum bestaetigten RT-3100-Pfad einzeln live validieren.
...
- NT530P mit dem vorhandenen BuiltIn-Schnittstellenprofil als validierten Tono/Pachy-Stand halten; optionalen JPG-Mehrfachanhang, Archiv-/Entfernen-Nachlauf und neutralen Monitoring-Kartenstatus nach Export separat testen.
...
- NIDEK RT6100 mit dem vorhandenen BuiltIn-Schnittstellenprofil praktisch validieren: Ausgabeordner im Bereich `Ausgabe an Geraet` auf den passenden MEM-200-Unterordner `DIRECT_RT_xx\TXT` setzen, aus AIS-Historienwerten eine RT-6100-Importdatei erzeugen, am Phoropter einlesen und die echte RT-Rueckgabe mit `Best` unter `6228` sowie `Full` unter `6227` pruefen. Die bereitgestellte OCR-XML-Datei ist bewusst nur als malformed Diagnosefixture geeignet; fuer die Praxisabnahme wird eine echte wohlgeformte RT-6100-Datei benoetigt.
...
7. Danach NIDEK NT530P als validierten Tono/Pachy-Stand halten und JPG-/Mehrfachanhang-Sonderfaelle separat pruefen; NIDEK Batch 6 XML-Varianten mit echten Rohdateien gegen die vorbereiteten BuiltIns pruefen; TOPCON CL300, TOPCON KR800S, TOPCON TRK2P und TOPCON CT1P als validierte Referenzkandidaten beibehalten, TOPCON Solos, TOPCON KR-1 und TOPCON CT800A praktisch abnehmen, H/V-Prisma sowie weitere SBJ-/KM-KRT-/Tonometrie-Funktionstestfaelle beobachten.
8. TOPCON CV5000 und NIDEK RT6100 bidirektional weiter live testen: erzeugte CV-5000- beziehungsweise RT-6100-Importdatei am Phoropter einlesen, Phoropter-Rueckgabe mit `Prescription`/`Best` unter `6228` und `Full Correction`/`Full` unter `6227` verarbeiten und besonders direkte Folgezyklen mit gleichnamiger neu geschriebener `Patient.gdt` sowie gleichnamig ueberschriebener/geaenderter Rueckgabedatei ohne Reset beobachten.
...
- NIDEK Batch 6: AR-1/AR-1S/AR-310A, LM-1800PD, NT-1/NT-1E/NT-1P/NT-510/NT-530 sind testseitig als XML-Varianten vorbereitet. Die naechste Entscheidung sind echte Rohdateien und praktische MEDISTAR-Abnahme; CEM-530, TONOREF II/III und script-/RDD-basierte Altgeraete bleiben ohne echte Daten bewusst Kandidaten.
...
- TOMEY-Referenzdaten: CF-2000, TL-2000C, TL-6000, TL-7000, MR-6000, TOP-1000, EM-3000 und EM-4000 sind als BuiltIns mit synthetischen Fixtures vorbereitet. EM nutzt `TomeyEmDeviceParser` und mappt Messwerte nach `6228`, Kommentare nach `6227` und Bild-/Dateiverweise nach `6302`. Die naechste Entscheidung ist Praxisrohdaten sammeln und gegen `TomeyDeviceParser` beziehungsweise `TomeyEmDeviceParser` validieren; AP-2500 und TAP-2000 bleiben Kandidaten ohne produktive BuiltIn-Freigabe. TAP-2000 wurde erneut geprueft, besitzt aber ohne echte Rueckgabe-/Live-Sendeframes noch keine belastbare produktive Freigabe.
