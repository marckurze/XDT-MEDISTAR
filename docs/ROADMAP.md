...
- `MEDISTAR + TOPCON TRK2P` ist als testseitig vollstaendiger TOPCON-Mehruntersuchungs-Kandidat vorhanden: echte XML-Fixtures mit `nsCommon`/`nsREF`/`nsKM`/`nsTM`, BuiltIn-Geraeteprofil, Exportprofil, Schnittstellenprofil, `6228` REF, `6221` KM, `6220` CCT/Pachy, `6205` Tono, optionaler `6227` SBJ-Pfad, selektiver Templatepaket-Test und Repair alter persistierter BuiltIns sind vorhanden. Praktische MEDISTAR-Abnahme bleibt offen.
...
Praxisprotokolle: `docs/E2E_TESTPROTOKOLL_MEDISTAR_ARK1S_XDT_ANHANG.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_AR360.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_LM7.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_CL300.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_KR800S.md` und `docs/PRAXISABNAHME_GERAETEFENSTER_V1.md`. Die vollstaendige Abarbeitung aller weiteren Testfaelle aus `docs/END_TO_END_TESTPLAN.md` bleibt als separater Schritt offen.
...
- TOPCON TRK2P: testseitig vorbereitet mit echten Serial0001-/Serial0135-XML-Fixtures, REF-`6228`, KM-`6221`, Pachy-`6220`, Tono-`6205`, optionalem SBJ-`6227`-Pfad und Templatepaket-Kandidat; praktische MEDISTAR-Abnahme offen.
- Fertige, auslieferbare Geraete-Templatepakete als ZIP-Artefakte. Fuer ARK1S, AR360, LM7, NT530P, CL300, KR800S, TRK2P und Dokumentanhang sind Referenz- beziehungsweise Kandidatendokumentation und reproduzierbare Export-/Import-Tests bereits vorhanden; dauerhafte ZIPs folgen erst nach `docs/TEMPLATEPAKET_RELEASE_REGEL.md`.
...
- NIDEK NT530P als naechsten praktischen MEDISTAR-Testkandidaten mit korrigiertem `6205`-/`6220`-Layout, JPG-Mehrfachanhaengen und sauberem Ordner-/Karten-Nachlauf erneut abnehmen; TOPCON CL300 und TOPCON KR800S als praktisch validierte TOPCON-Referenzkandidaten beibehalten, H/V-Prisma beziehungsweise SBJ-Funktionstestfaelle weiter beobachten und TOPCON TRK2P mit beiden XML-Fixtures praktisch pruefen.
...
- TOPCON TRK2P mit dem vorhandenen BuiltIn-Schnittstellenprofil praktisch validieren: Serial0001 deckt REF/KM/TM ohne CCT ab, Serial0135 deckt REF/KM/TM mit CorrectedIOP-CCT-Fallback fuer `6220` ab; SBJ bleibt optional und ist in den aktuellen Fixtures nicht enthalten.
...
7. Danach NIDEK NT530P praktisch in MEDISTAR mit korrigierter `6205`/`6220`-Ausgabe und JPG-Mehrfachanhaengen validieren; TOPCON CL300 und TOPCON KR800S als validierte Referenzkandidaten beibehalten, H/V-Prisma sowie weitere SBJ-Funktionstestfaelle beobachten und TOPCON TRK2P mit Serial0001/Serial0135 live pruefen.
8. MEDISTAR + Dokumentanhang und MEDISTAR + Manuelle Dokumentuebergabe im Live-Test mit mehreren Anhangdateien, TopMost-Dialog, Zwischenablage/Dateiauswahl/Drag-&-Drop und pro-Datei-`6304`-Text pruefen.
9. Restliche E2E-Testfälle mit realen Testordnern ausführen und mit `docs/E2E_TESTPROTOKOLL_TEMPLATE.md` protokollieren.
10. Read-only Aktivierungsassistent nur als geparkten Regressionsstand weiterfuehren.
11. Lizenzsignatur- und Installer-Themen erst nach weiterem Profil-/Template-Nutzen priorisieren.
