...
- `MEDISTAR + TOPCON KR800S` ist als testseitig direkt nutzbarer TOPCON-Mehruntersuchungs-Kandidat vorhanden: echte Shift-JIS-XML-Fixtures mit `nsCommon`/`nsREF`/`nsKM`/`nsSBJ`, BuiltIn-Geraeteprofil, Exportprofil, Schnittstellenprofil, `6228` REF, `6221` KM, konservative `6227` SBJ-Zeilen mit getrennten Header-/Messwertzeilen und selektiver Templatepaket-Test sind vorhanden. Alte persistierte BuiltIn-Exportprofile mit leeren Einzelplatzhalter-Templates oder KM ueber `6228` werden gezielt repariert. Praktische MEDISTAR-Validierung nach dem Fix bleibt offen.
...
Praxisprotokolle: `docs/E2E_TESTPROTOKOLL_MEDISTAR_ARK1S_XDT_ANHANG.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_AR360.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_LM7.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_CL300.md` und `docs/PRAXISABNAHME_GERAETEFENSTER_V1.md`. Die vollständige Abarbeitung aller weiteren Testfälle aus `docs/END_TO_END_TESTPLAN.md` bleibt als separater Schritt offen.
...
- TOPCON KR800S: testseitig angebunden; leere Live-Platzhalter durch BuiltIn-Repair behoben; praktische MEDISTAR-Abnahme offen.
- TOPCON TRK2P.
- Fertige, auslieferbare Geraete-Templatepakete als ZIP-Artefakte. Fuer ARK1S, AR360, LM7, NT530P und Dokumentanhang sind Referenz- beziehungsweise Kandidatendokumentation und reproduzierbare Export-/Import-Tests bereits vorhanden; dauerhafte ZIPs folgen erst nach `docs/TEMPLATEPAKET_RELEASE_REGEL.md`.
...
- NIDEK NT530P als naechsten praktischen MEDISTAR-Testkandidaten mit korrigiertem `6205`-/`6220`-Layout, JPG-Mehrfachanhaengen und sauberem Ordner-/Karten-Nachlauf erneut abnehmen; TOPCON CL300 als praktisch validierten TOPCON-Referenzkandidaten beibehalten und H/V-Prisma weiter beobachten; TOPCON KR800S mit REF/KM/SBJ als naechsten TOPCON-Praxiskandidaten testen.
...
- TOPCON KR800S mit dem vorhandenen BuiltIn-Schnittstellenprofil praktisch in MEDISTAR testen: Serial0036 und Serial0426 decken `6228` REF, `6221` KM und konservative `6227` SBJ-Ausgabe mit getrennten Header-/Messwertzeilen ab.
...
7. Danach NIDEK NT530P praktisch in MEDISTAR mit korrigierter `6205`/`6220`-Ausgabe und JPG-Mehrfachanhaengen validieren; TOPCON CL300 als validierten Referenzkandidaten beibehalten, H/V-Prisma weiter beobachten und TOPCON KR800S mit echten REF/KM/SBJ-Dateien praktisch abnehmen. TRK2P erst mit belastbaren Beispiel- und Testdaten priorisieren.
