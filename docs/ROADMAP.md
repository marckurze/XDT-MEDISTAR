...
- `MEDISTAR + TOPCON KR800S` ist als testseitig direkt nutzbarer TOPCON-Mehruntersuchungs-Kandidat vorhanden: echte Shift-JIS-XML-Fixtures mit `nsCommon`/`nsREF`/`nsKM`/`nsSBJ`, BuiltIn-Geraeteprofil, Exportprofil, Schnittstellenprofil, `6228` REF, `6221` KM, konservative `6227` SBJ-Zeilen und selektiver Templatepaket-Test sind vorhanden. Alte persistierte BuiltIn-Exportprofile mit leeren Einzelplatzhalter-Templates oder KM ueber `6228` werden gezielt repariert. Praktische MEDISTAR-Validierung nach dem Fix bleibt offen.
...
- TOPCON KR800S mit dem vorhandenen BuiltIn-Schnittstellenprofil praktisch in MEDISTAR testen: Serial0036 und Serial0426 decken `6228` REF, `6221` KM und konservative `6227` SBJ-Ausgabe ab.
