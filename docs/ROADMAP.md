...
- `MEDISTAR + Dokumentanhang` ist als AttachmentOnly-/Dokumentgeraete-V1-Kandidat vorhanden: AIS-Datei plus eine oder mehrere stabile Anhangdateien, optionaler ein- oder mehrzeiliger `6227`-Dokumentationstext und je Datei eigene `6302`-`6305`-Linkfelder; es werden keine Messwertfelder erzeugt. Fuer mehrere nacheinander geschriebene Dateien gibt es profilbezogen `Abschluss nach Wartezeit` und `Manuell bestaetigen`; im manuellen Modus bleibt der Dialog offen und sammelt weitere Dateien bis `Uebertragen`. Der Dokument-Importordner ist die Anhangquelle, alte deaktivierte technische Anhang-Flags blockieren AttachmentOnly nicht mehr.
...
- Generisches Dokumentgeraet / AttachmentOnly: BuiltIn-Schnittstellenprofil, sichtbare Option `Dokumentationstext erfassen`, optionaler `6227`-Text, mehrere `6302`-`6305`-Anhaenge, XML als Anhang statt Messwertdatei, Ruhezeit-/Bestaetigungsabschluss und Templatepaket-Kandidat sind testseitig vorhanden; praktische MEDISTAR-Abnahme offen.
- TOPCON CL300.
- TOPCON KR800.
- TOPCON TRK2P.
...
- Generisches Dokumentgeraet als AttachmentOnly-V1 praktisch testen: mehrere Anhaenge, Ruhezeit nach letzter Datei, manueller Uebertragen-Dialog, optionaler `6227`-Text, keine `6228`/`6205`/`6220`-Messwertausgabe.
...
- MEDISTAR + Dokumentanhang mit PDF/JPG/XML/MP4/MP3/WAV-Dateien praktisch pruefen, inklusive Ruhezeitmodus und manuellem Bestaetigungsmodus; Drag-&-Drop, Vorschau, OCR und pro-Datei-Kommentare bleiben separate spaetere Themen.
...
8. MEDISTAR + Dokumentanhang im Live-Test mit mehreren Anhangdateien, Ruhezeit-/Bestaetigungsabschluss und optionalem `6227`-Text pruefen.
