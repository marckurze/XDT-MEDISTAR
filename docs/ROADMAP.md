...
- `MEDISTAR + Manuelle Dokumentuebergabe` ist als eigener V1-Kandidat vorhanden: Erst eine AIS-Datei startet den Uebertragen-Dialog mit leerer Liste, das gruene Geraeteanbindungsfenster bleibt angedockt, der Anwender fuegt Dateien per Drag-&-Drop, Dateiauswahl oder Datei-Zwischenablage hinzu, und erst `Uebertragen` exportiert je Datei eigene `6302`-`6305`-Linkfelder. Ein Geraete-Importordner ist fuer diesen Modus nicht erforderlich.
