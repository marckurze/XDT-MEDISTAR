...
- `InterfaceProfileActivationExecutorStub` ist als defensive Backend-Stufe vorhanden. Er bewertet Preconditions und liefert Statuswerte; im `ValidateOnly`-Modus kann er optional den ActivationProfileStore fuer frisches Laden und Save-DryRun-Pruefung nutzen. Er setzt aber kein `IsActive`, speichert nichts, startet keine Verarbeitung und ist nicht an die UI angebunden.
