...
- Exportregeln koennen in der Baukasten-Arbeitskopie per Plus/Muelltonne ergaenzt oder entfernt werden; feste Notizen ohne `SourcePath`, Undo und Beispielwerte in Platzhaltern sind als V1-Bedienhilfen umgesetzt. Geraeteplatzhalter werden aus der passenden geladenen Geraetedatei befuellt und bei Profilwechsel gegen Cross-Device-Werte geschuetzt; die zentrale Kompatibilitaetspruefung akzeptiert BuiltIn-Modell-Aliase und bleibt fuer UserDefined-/Entwurfsgeraete bewusst permissiv, damit neue Geraete im Baukasten gebaut werden koennen. `Templatepaket laden` zeigt bis zur lokalen Paketbibliothek eine sichtbare V1-Hinweismeldung.
