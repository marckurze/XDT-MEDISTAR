...
- `MEDISTAR + TOPCON CV5000` ist als erster bidirektionaler TOPCON-Phoropter-Kandidat testseitig vorbereitet: AIS-Historienparser fuer MEDISTAR-Karteikartenzeilen, CV-5000-Import-XML-Writer, CV-5000-Rueckgabeparser, BuiltIn-Geraeteprofil, Exportprofil, Schnittstellenprofil, `6228`-Rueckgabe, schnittstellenprofilbasierte Ausgabe-an-Geraet-Konfiguration, selektiver Templatepaket-Test und Fixture-Protokoll sind vorhanden. Das Geraeteprofil markiert nur die bidirektionale Faehigkeit; Ausgabeordner, Dateiname und Format liegen im Schnittstellenprofil. Praktische MEDISTAR-/CV-5000-Livevalidierung bleibt offen.
...
- AlreadyProcessed-/Duplikat-Nachlauf ist generisch abgesichert: bekannte bereits erfolgreich verarbeitete AIS-/Geraetedateien werden bei verfuegbarer Archiv-, Fehlerordner- oder Entfernen-Regel aus dem aktiven Importordner herausgenommen, ohne unbekannte Dateien anzufassen.
...
Praxis- und Fixture-Protokolle: `docs/E2E_TESTPROTOKOLL_MEDISTAR_ARK1S_XDT_ANHANG.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_AR360.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_LM7.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_CL300.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_KR800S.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_TRK2P.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_CT1P.md`, `docs/E2E_TESTPROTOKOLL_MEDISTAR_TOPCON_CV5000.md` und `docs/PRAXISABNAHME_GERAETEFENSTER_V1.md`. Die vollstaendige Abarbeitung aller weiteren Testfaelle aus `docs/END_TO_END_TESTPLAN.md` bleibt als separater Schritt offen.
...
- TOPCON CV5000: testseitig vorbereiteter bidirektionaler Phoropter-Kandidat mit AIS-Historienparser, CV-5000-Import-XML-Writer, Rueckgabeparser, `6228`-Export, BuiltIn-Profilen, Schnittstellenprofil-Konfiguration fuer Ausgabe an Geraet, Templatepaket-Kandidat und Fixture-Protokoll; praktische MEDISTAR-/CV-5000-Livevalidierung offen.
- Fertige, auslieferbare Geraete-Templatepakete als ZIP-Artefakte. Fuer ARK1S, AR360, LM7, NT530P, CL300, KR800S, TRK2P, CT1P, CV5000 und Dokumentanhang sind Referenz- beziehungsweise Kandidatendokumentation und reproduzierbare Export-/Import-Tests bereits vorhanden; dauerhafte ZIPs folgen erst nach `docs/TEMPLATEPAKET_RELEASE_REGEL.md`.
...
- TOPCON CV5000 mit dem vorhandenen BuiltIn-Schnittstellenprofil praktisch validieren: Ausgabeordner im Bereich `Ausgabe an Geraet` setzen, AIS-Historienauswahl aus `Patient_mit_Phoropter_Daten.XDT`, Schreiben der CV-5000-Import-XML, Einlesen am Phoropter und Rueckgabe ueber `6228`. Die Fixture-Tests fuer Importwriter, UI-Sichtbarkeit und Rueckgabeparser sind vorhanden.
...
7. Danach NIDEK NT530P praktisch in MEDISTAR mit korrigierter `6205`/`6220`-Ausgabe und JPG-Mehrfachanhaengen validieren; TOPCON CL300, TOPCON KR800S, TOPCON TRK2P und TOPCON CT1P als validierte Referenzkandidaten beibehalten, H/V-Prisma sowie weitere SBJ-Funktionstestfaelle beobachten.
8. TOPCON CV5000 bidirektional live testen: im Schnittstellenprofil Ausgabeordner setzen, Auswahl historischer MEDISTAR-Messwerte, CV-5000-Importdatei und Phoropter-Rueckgabe ueber `6228`.
