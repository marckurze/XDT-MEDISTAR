...
- TOPCON CV5000: bidirektionaler Phoropter-Kandidat mit AIS-Historienparser, angebundenem Auswahlfenster `Werte an Phoropter uebergeben`, CV-5000-Import-XML-Writer, Rueckgabeparser, `6228`-Prescription-Export, CV-5000-spezifischem `6330`-Full-Correction-Export, BuiltIn-Profilen, Schnittstellenprofil-Konfiguration fuer Ausgabe an Geraet, Templatepaket-Kandidat und Fixture-Protokoll; Rueckweg mit MEDISTAR-Historien-AIS ist abgesichert, finaler MEDISTAR-Import der Rueckgabedatei offen.
...
- TOPCON CV5000 mit dem vorhandenen BuiltIn-Schnittstellenprofil weiter praktisch validieren: Ausgabeordner im Bereich `Ausgabe an Geraet` setzen, AIS-Historienauswahl aus dem angebundenen Dialog mit `Patient_mit_Phoropter_Daten.XDT`, Schreiben der CV-5000-Import-XML, Einlesen am Phoropter, Rueckgabe mit `Prescription` unter `6228` und `Full Correction` unter CV-5000-spezifischem `6330` sowie finalen MEDISTAR-Import pruefen. Die Fixture-Tests fuer Importwriter, UI-Sichtbarkeit, Dialog-Trigger-Policy, Rueckgabeparser und toleranten Rueckweg mit MEDISTAR-Historien-AIS sind vorhanden.
...
8. TOPCON CV5000 bidirektional weiter live testen: erzeugte CV-5000-Importdatei am Phoropter einlesen, Phoropter-Rueckgabe mit `Prescription` unter `6228` und `Full Correction` unter CV-5000-spezifischem `6330` verarbeiten und den finalen MEDISTAR-Import der Rueckgabedatei bestaetigen.
