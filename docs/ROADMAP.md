...
- Der Tab `Schnittstellenprofile` ist fuer die naechsten Geraete-Batches bereinigt: Hersteller- und AIS-Filter erleichtern die Profilwahl, aktive Schnittstellenprofile zaehlen immer als Geraeteanbindung, der fruehere Lizenzpflicht-Haken ist entfernt, und `AIS Ausgabe Info` zeigt eine reine AIS-Felduebersicht inklusive `8402`-Untersuchungsart-Hinweis und empfohlenen Defaultwerten. Reine Dokumentgeraete markieren `6302`, `6303` und `6305` als Karteikarten-relevant; Messgeraete mit optionalem Anhang behalten diese Felder optional. Das `6305`-Layout laesst die Ordnerbuttons sichtbar. Neue BuiltIn-/Template-/Repair-Profile starten mit Ordnerbereinigung aktiv, Archivierung aus und Fehlerablage aktiv; UserDefined-Profile behalten bewusste Einstellungen.
