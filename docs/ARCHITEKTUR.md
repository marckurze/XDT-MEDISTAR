...
Für das Warten auf vollständige Verarbeitungspakete ist ein zweistufiges Modell vorbereitet. Ein Paket besteht aus AIS-Datei, Gerätedatei und optionalem oder verpflichtendem XDT-Anhang. Phase 1: Eine erkannte stabile AIS-Datei wartet auf eine stabile Gerätedatei; die Wartezeit ist pro Schnittstellenprofil konfigurierbar, Standard `10` Minuten. Kommt vor der Gerätedatei eine neuere AIS-Datei, ersetzt sie den wartenden Auftrag. Phase 2: Erst ab vollständigem AIS-/Geräte-Dateipaar startet die XDT-Anhang-Wartezeit. Standard ist `optional`, die Standard-Wartezeit beträgt 30 Sekunden. Optional bedeutet: Nach Ablauf der Wartezeit darf die Messwertverarbeitung ohne Anhang fortgesetzt werden. Pflicht bedeutet: Ohne eindeutigen Anhang wird die Verarbeitung blockiert. Bei Pflicht-Timeout wird das konkrete Paket terminal als Fehlerfall abgeschlossen und nur die bekannten AIS-/Gerätedateien dieses Pakets werden gemäß Fehler-/Archivoptionen verschoben oder kopiert; unbekannte Dateien und Exportordner werden nicht pauschal bereinigt. Mehrere unterstützte Anhänge bleiben immer unsicher und werden nicht automatisch zugeordnet.
...
Als Grundlage fuer einen spaeteren Aktivierungsassistenten bewertet der `InterfaceProfileActivationEvaluationService` importierte beziehungsweise UserDefined-Schnittstellenprofile read-only. Er prueft Profilgrunddaten, BuiltIn-Schutz, AIS-/Geraete-/Exportprofil-Abhaengigkeiten, erforderliche Ordner, XDT-Anhang-Konfiguration und Lizenzrelevanz. Das Ergebnis unterscheidet `Ready`, `ReadyWithWarnings` und `Blocked` und liefert Blocker, Warnungen und Hinweise. Im Tab `Schnittstellenprofile` gibt es dafuer eine erste reine Pruefvorschau mit Gesamtstatus, Aktivierbarkeit, strukturierter Ordnerpruefung, strukturierter XDT-Anhang-Konfiguration und allen Einzelpruefungen. Der Einstieg `Aktivierung vorbereiten` zeigt zusaetzlich eine kompakte Zusammenfassung der Aktivierungsbereitschaft mit wichtigsten Blockern und Warnungen, die technische Guard-Entscheidung und die spaeter erforderliche Warnungsbestaetigung rein lesend an. Service und UI-Vorschau legen keine Ordner an, schreiben keine Dateien, aendern keine Profile, aktivieren keine importierten Schnittstellenprofile und starten keine Verarbeitung.

Ergaenzend dazu bereitet der `InterfaceProfileActivationGuardService` eine technische Schutzschicht fuer eine spaetere echte Aktivierung vor. Der Guard nimmt ein Schnittstellenprofil, eine aktuelle Aktivierungsbewertung und eine spaetere Warnungsbestaetigung entgegen und entscheidet nur, ob eine Aktivierungsanforderung technisch weitergehen duerfte. Fehlende Bewertungen, blockierte Bewertungen, BuiltIn-Profile und nicht als UserDefined markierte Profile werden konservativ blockiert. `ReadyWithWarnings` erfordert eine bewusste Warnungsbestaetigung, bevor der Guard `AllowedWithWarnings` meldet. Im aktuellen Dialog wird der Guard mit nicht bestaetigten Warnungen nur angezeigt; es gibt keine Checkbox, keine Speicherung und keinen produktiven Aktivierungsbutton. Auch der Guard hat keine Seiteneffekte: Er speichert nichts, aendert keine Flags, fuehrt keine Datei-/Ordneroperationen aus und startet keine Verarbeitung.

Der `InterfaceProfileActivationWarningConfirmationService` modelliert ergaenzend rein intern, welche Warnungen spaeter vor einer Aktivierung bewusst bestaetigt werden muessten. Er erzeugt aus der aktuellen Aktivierungsbewertung einzelne bestaetigungspflichtige Warning-Items und lehnt eine Bestaetigungsanforderung bei fehlender Bewertung, Blockern, BuiltIn-Profilen, nicht-UserDefined-Profilen oder uneindeutigem Status ab. Blocker und Hinweise werden nicht bestaetigbar gemacht. Der Dialog `Aktivierung vorbereiten` zeigt diese Information nur als Vorschau: `ReadyWithWarnings` listet die bestaetigungspflichtigen Warnungen, `Ready` meldet keine erforderliche Bestaetigung, `Blocked` meldet eine noch nicht moegliche Bestaetigung. Es gibt weiterhin keine Checkbox, keinen Bestaetigungsbutton und keinen produktiven Aktivierungsbutton. Der Service speichert keine Bestaetigung, aendert keine Profile, fuehrt keine Datei-/Ordneroperationen aus und startet keine Verarbeitung.
...
`InterfaceProfileActivationExecutorStub` ist eine defensive Backend-Implementierung ohne produktive Wirkung: Er bewertet die uebergebenen Preconditions und gibt Statuswerte zurueck, setzt aber kein `IsActive`, speichert nichts, aendert keine Profile, startet keine Verarbeitung und fuehrt keine Datei-/Ordneroperationen aus. Optional kann der Stub im `ValidateOnly`-Modus einen `IInterfaceProfileActivationProfileStore` nutzen, um das Zielprofil frisch zu laden und BuiltIn/UserDefined naeher an der echten Projektstruktur zu pruefen. Optional kann er danach die finale Pruefkette mit `InterfaceProfileActivationEvaluationService`, `InterfaceProfileActivationGuardService`, `InterfaceProfileActivationWarningConfirmationService` und `InterfaceProfileActivationPlanService` simulieren, sofern diese Services und ein aktueller `ProfileCatalog` injiziert wurden. Ohne diese Abhaengigkeiten meldet er MissingCapabilities; in `Activate`/`Deactivate` wird der Store weiterhin nicht fuer produktive Aktionen genutzt.
