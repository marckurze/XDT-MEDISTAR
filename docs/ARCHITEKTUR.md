...
Als Grundlage fuer einen spaeteren Aktivierungsassistenten bewertet der `InterfaceProfileActivationEvaluationService` importierte beziehungsweise UserDefined-Schnittstellenprofile read-only. Er prueft Profilgrunddaten, BuiltIn-Schutz, AIS-/Geraete-/Exportprofil-Abhaengigkeiten, erforderliche Ordner, XDT-Anhang-Konfiguration und Lizenzrelevanz. Das Ergebnis unterscheidet `Ready`, `ReadyWithWarnings` und `Blocked` und liefert Blocker, Warnungen und Hinweise. Der Service legt keine Ordner an, schreibt keine Dateien, aendert keine Profile, aktiviert keine importierten Schnittstellenprofile und startet keine Verarbeitung.
