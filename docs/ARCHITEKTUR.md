...
### 1.1 XDT-Baukasten und Profilverwaltung

Der manuelle Entwurfs- und Vorschautest befindet sich im Tab `XDT-Baukasten`:

1. AIS-GDT-Datei auswählen
2. Gerätedatei oder RS232-Rohdaten laden
3. Patientendaten und Messwerte einlesen
4. Mapping-/Exportprofil als Arbeitskopie anwenden
5. Vorschau für `Roh-XDT`, `Ansicht im AIS`, `Geräteausgabe` und `Diagnose` erzeugen
6. Regeln, Platzhalter und Templatepakete im Baukasten ohne produktiven Schreibzugriff prüfen
...
Der frühere Tab `Profile & Templates` ist entfernt. Die Profilpflege liegt im Tab `Profilverwaltung`: AIS-, Geräte-, Export- und Schnittstellenprofile sowie lokale Templatepakete und Baukasten-Templates werden dort gesucht, gefiltert, dupliziert, umbenannt, gelöscht, exportiert beziehungsweise zur Bearbeitung in den passenden Zielbereich geöffnet. `XDT-Baukasten`, `Profilverwaltung` und `Schnittstellenprofile` sind damit bewusst getrennt: Vorschau/Entwurf, Pflege/Wartung und konkrete Praxisanbindung.
...
- `AttachmentTransferService`, isoliert vorbereitet für sichere Übertragung explizit übergebener Anhangdateien ohne Überschreiben
...
- `AttachmentExternalLinkPreparationService`, isoliert vorbereitet für die explizite Orchestrierung aus Ziel-Dateiname, Copy/Move-Transfer, externen AIS-Linkfeldwerten und XDT-Feldcode/Wert-Paaren; in der automatischen Paarverarbeitung je stabilem unterstütztem Anhang einzeln aufgerufen
...
- `AttachmentAutoCandidateSelectionService`, isoliert vorbereitet für die sichere automatische Kandidatenentscheidung; ein oder mehrere stabile unterstützte Anhangdateien werden sortiert ausgewählt, instabile Dateien blockieren bis zur Stabilität beziehungsweise bis zur bestehenden Timeoutentscheidung
- `AttachmentPackageDecisionService`, isoliert vorbereitet für die Entscheidung, ob auf optionale oder verpflichtende XDT-Anhänge gewartet, ohne Anhang fortgesetzt, mit stabilen Anhängen verarbeitet oder wegen fehlendem Pflichtanhang blockiert werden soll
...
- Baukasten-Testpfad im Tab `XDT-Baukasten`, der eine explizit ausgewählte Anhangdatei testweise vorbereitet und Ziel-Dateiname, Zielpfad sowie Feldcode/Wert-Paare `6302` bis `6305` anzeigt; er ist nicht Teil des periodischen Scans und verändert kein Exportprofil
...
Als Grundlage fuer eine schlanke spaetere V1-Aktivierung bewertet der `InterfaceProfileActivationEvaluationService` importierte beziehungsweise UserDefined-Schnittstellenprofile read-only. Er prueft Profilgrunddaten, BuiltIn-Schutz, AIS-/Geraete-/Exportprofil-Abhaengigkeiten, erforderliche Ordner, XDT-Anhang-Konfiguration und Lizenzrelevanz. Das Ergebnis unterscheidet `Ready`, `ReadyWithWarnings` und `Blocked` und liefert Blocker, Warnungen und Hinweise.

Im Tab `Schnittstellenprofile` zeigt der Bereich `Pruefung vor Aktivierung` Gesamtstatus, Aktivierbarkeit, Zaehler, strukturierte Ordnerpruefung, strukturierte XDT-Anhang-Konfiguration und alle Pruefpunkte. Der Dialog `Aktivierung vorbereiten` ist weiterhin ein scrollbares Preview-Fenster mit OK-/Schliessen-Aktion, zeigt aber nur noch die V1-relevanten Informationen: Profilname, Status, `Aktivierbar nach V1`, technische Guard-Entscheidung, Blocker, Warnungen, Hinweise und Sicherheitshinweis. Es gibt keinen Aktivieren-Button, keine Checkbox und keine produktive Wirkung.

Der `InterfaceProfileActivationGuardService` ist die schlanke technische Schutzschicht. Er blockiert fehlende Bewertungen, blockierte Bewertungen, BuiltIn-Profile, nicht als UserDefined markierte Profile und `ReadyWithWarnings`. Fuer V1 gilt: Nur `Ready` ohne Warnungen kann technisch `Allowed` werden. `ReadyWithWarnings` wird angezeigt, aber nicht bestaetigt und nicht als Produktivpfad vorbereitet.
...
- `InterfaceProfileActivationGuardDecision`: `Allowed`, `Blocked`, `Unknown`

Wichtig fuer spaetere Erweiterungen: Es gibt weiterhin keinen produktiven Aktivieren-Button, keine automatische Aktivierung, keine Aenderung an `IsActive` oder `IsAttachmentProcessingEnabled`, keine Profil-Speicherung und keine produktive Warnungsbestaetigung. BuiltIn-Profile bleiben direkt blockiert; die spaetere V1 ist auf bewusst kontrollierte UserDefined-Schnittstellenprofile mit Status `Ready` ohne Warnungen ausgerichtet. Importierte Schnittstellenprofile bleiben bis zu einer spaeteren expliziten Aktivierung inaktiv.
...
Als reine technische Leitplanke fuer eine spaetere echte Aktivierung ist ein Interface-/Model-Skelett vorhanden:
...
- `InterfaceProfileActivationProfileLoadResult`
- `InterfaceProfileActivationProfileSaveRequest`
- `InterfaceProfileActivationProfileSaveResult`
- `InterfaceProfileActivationProfileStoreStatus`
- `InterfaceProfileActivationProfileStoreStub`
- `InterfaceProfileActivationProfileCatalogStore`

Das Skelett ist nicht in die UI eingebunden. `InterfaceProfileActivationExecutorRequest` traegt nur noch den V1-noetigen Kontext: Zielprofil-ID/-Name, Modus `ValidateOnly` oder `Activate`, Quelle und optionalen Zeitpunkt sowie Preview-Evaluation/-Guard als Kontext. Konkrete Loader-/Store-Services werden nicht in den Request gelegt; ein spaeterer produktiver Executor muss sie nach Projektmuster selbst erhalten und direkt vor Speicherung frisch laden. `InterfaceProfileActivationExecutorResult` kann ausdruecken, dass frisches Laden, sichere UserDefined-Speicherung und finale Bewertung noch erforderlich sind.

`InterfaceProfileActivationExecutorStub` ist eine defensive Backend-Implementierung ohne produktive Wirkung: Er bewertet Preconditions und gibt Statuswerte zurueck, setzt aber kein `IsActive`, speichert nichts, aendert keine Profile, startet keine Verarbeitung und fuehrt keine Datei-/Ordneroperationen aus. Im `ValidateOnly`-Modus kann der Stub optional einen `IInterfaceProfileActivationProfileStore` nutzen, um das Zielprofil frisch zu laden und BuiltIn/UserDefined naeher an der echten Projektstruktur zu pruefen. Optional kann er danach die finale V1-Pruefkette mit `InterfaceProfileActivationEvaluationService` und `InterfaceProfileActivationGuardService` simulieren. `ReadyWithWarnings` blockiert auch hier. Im `Activate`-Modus bleibt der Stub ausdruecklich `NotImplemented` und nutzt den Store nicht produktiv.

Die executor-nahe Store-Abstraktion bereitet nur den spaeteren Sicherheitsrand vor: `LoadFreshUserDefinedProfile` soll spaeter ein Schnittstellenprofil per ID frisch laden und BuiltIn/Nicht-UserDefined blockierbar machen; `SaveUserDefinedProfile` modelliert die spaetere gezielte UserDefined-Speicherung. `InterfaceProfileActivationProfileStoreStub` arbeitet nur mit einem uebergebenen In-Memory-Bestand und speichert nichts. `InterfaceProfileActivationProfileCatalogStore` ist die erste echte Adapterstufe gegen `ProfileCatalogService` und `AppDataPaths`: Laden liest den bestehenden Profilkatalog, Save bleibt aber strikt ValidateOnly/DryRun. Der Stub gibt Save-DryRun erst frei, wenn die nicht-produktive finale ValidateOnly-Pruefkette erfolgreich war; der Adapter ruft `SaveInterfaceProfileDefinition` nicht auf, schreibt keine JSON-Datei, aendert keine Profile und setzt kein `IsActive`.

Die vorgesehenen Executor-Statuswerte sind `NotAvailable`, `NotImplemented`, `Blocked`, `ReadyButNotExecuted`, `Failed` und `Success`. `Success` ist nur fuer eine spaetere Implementierung reserviert; im aktuellen Stand gibt es keinen Codepfad, der eine Aktivierung ausfuehrt oder speichert.

Vor einer produktiven V1-Implementierung muessen fachlich und technisch nur noch die schlanken Kernpunkte geklaert werden: `IsActive` als finaler Schreibpunkt, konkrete sichere UserDefined-Store-Methode, frische Profilkatalog-Ladung, finale Direktpruefung unmittelbar vor Speichern und die UI-Position des spaeteren Buttons `Profil aktivieren`. Warnungsbestaetigung, Audit, Rollenmodell, Deaktivierung und Paketstatus-Sonderregeln sind bewusst nicht Teil von V1.
