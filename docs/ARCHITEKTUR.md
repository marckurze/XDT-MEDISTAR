...
Als Pilot kann eine Monitoring-Karte geraetespezifisch vom Radar abweichen. Fuer TOPCON CV-5000/CV-5000S liefert das Displaymodell `UsesPilotDeviceVisual`, `DeviceType` und optional `DeviceImagePath`; die WPF-Oberflaeche zeigt dann Geraetebild, Infoblock und pulsierende Statuskugel. Die Kugel ist eine reine Darstellung auf Basis von `ScanIntervalSeconds` und Dateieingangsstatus; sie aendert keine Scan- oder Exportlogik. Fuer andere Geraete bleibt der Radar-Zweig aktiv, bis ein spaeterer Rollout bewusst erfolgt.
