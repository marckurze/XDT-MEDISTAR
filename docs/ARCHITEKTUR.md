...
Die executor-nahe Store-Abstraktion bereitet nur den spaeteren Sicherheitsrand vor: `LoadFreshUserDefinedProfile` soll spaeter ein Schnittstellenprofil per ID frisch laden und BuiltIn/Nicht-UserDefined blockierbar machen; `SaveUserDefinedProfile` modelliert die spaetere gezielte UserDefined-Speicherung. Die aktuelle Implementierung ist nur `InterfaceProfileActivationProfileStoreStub`: Sie arbeitet mit einem uebergebenen In-Memory-Bestand, fuehrt keine Profilkatalog- oder Dateioperationen aus und gibt fuer Speichern weiterhin `SaveNotImplemented` zurueck. Der bestehende produktive Profilkatalog bleibt `ProfileCatalogService` mit `AppDataPaths`; eine echte Adapterimplementierung ist noch nicht angebunden.
