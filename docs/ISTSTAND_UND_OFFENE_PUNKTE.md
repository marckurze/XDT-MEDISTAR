...
- Vor einem spaeteren Installer ist die Installations-/Update-Datenpolitik dokumentiert (`docs/INSTALLATION_UPDATE_DATENPOLITIK.md`) und als `XdtBoxInstallationDataPolicy` testseitig abgesichert. App-Dateien und BuiltIns duerfen aktualisiert/repariert werden; Kundendaten unter `%LocalAppData%\XdtDeviceBridge`, UserDefined-Profile, Baukasten-Templates, lokale Geraetebilder, `device-image-overrides.json`, Lizenzdateien inklusive `license.xdtboxlic`, UI-Komfortdaten und Backups bleiben erhalten. Deinstallation folgt Variante B: Kundendaten bleiben standardmaessig erhalten und externe Praxisordner werden nie blind geloescht.
