...
- Aktivierungsassistent fuer importierte Schnittstellenprofile; erste read-only Backend-Bewertung ist vorhanden, UI und produktive Aktivierung bleiben offen
...
| hoch | Templatepaket-Import Aktivierungsassistent | Sicherer Import als UserDefined vorhanden; Backend-Bewertung fuer importierte Schnittstellenprofile prueft Abhaengigkeiten, Ordner, XDT-Anhang-Konfiguration und Lizenzhinweise read-only. | Wizard-/UI-Schritt, bewusste Benutzerfreigabe und eigentliche Aktivierung fehlen weiterhin. | Kleine UI fuer "importiertes Schnittstellenprofil pruefen" vorbereiten, noch ohne automatische Aktivierung. | Importierte Profile bleiben zwar sicher, aber fuer Anwender noch nicht komfortabel produktiv nutzbar. | TemplateImport Executor, Profilkatalog, InterfaceProfileActivationEvaluationService |
