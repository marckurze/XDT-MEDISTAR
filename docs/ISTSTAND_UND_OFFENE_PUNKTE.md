...
- stabilisierte Grundlage fuer abdockbare Geraeteanbindungsfenster im Tab `Verarbeitung`: Abdocken/Andocken ueber transparenten Symbolbutton `🗗`, Pin/TopMost `🔝` pro Floating-Fenster, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, sicheres X-Schliessen, Start-Wiederherstellung nach sicherer MainWindow-Anzeige, Radar pro Schnittstellenprofil, erste automatische Oeffnung bei Monitoring-Aktivitaet und akustisches Signal bei relevanter Dateiaktivitaet
...
| Abdockbare Geraeteanbindungsfenster | UI-Grundlage testseitig fuer State, Persistenz, verzoegerte Wiederherstellung, Auto-Abdocken und akustisches Signal bei Monitoring-Aktivitaet abgesichert | `InterfaceProfileFloatingWindowStateServiceTests`, `InterfaceProfileFloatingWindowStateRepositoryTests`, `InterfaceProfileFloatingWindowRestoreGateTests`, `InterfaceProfileAutoDetachServiceTests`, `InterfaceProfileNotificationSoundServiceTests` |
