...
- stabilisierte Grundlage fuer abdockbare Geraeteanbindungsfenster im Tab `Verarbeitung`: Abdocken/Andocken ueber transparenten Symbolbutton `🗗`, Pin/TopMost `🔝` pro Floating-Fenster, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, sicheres X-Schliessen, Start-Wiederherstellung nach sicherer MainWindow-Anzeige, Radar pro Schnittstellenprofil, automatische Oeffnung bei Monitoring-Aktivitaet, akustisches Signal bei relevanter Dateiaktivitaet und automatisches Zurueckandocken nach terminalem Abschluss
