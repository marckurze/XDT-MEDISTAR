...
- NIDEK RT-2100 / RT-3100 / RT-5100 ist als gemeinsame serielle RS232-Phoropterfamilie vorbereitet und im produktiven Ablauf angebunden. Die PDF-Auswertung bestaetigt SH/SX/EB/ET/CR-Steuerzeichen, RT-2100 Type1 mit 2400 7E2 sowie RT-3100/RT-5100 Type1 2400 7E2 und Type2 9600 8O1. BuiltIn-Profile, MEDISTAR-Exportprofile, Schnittstellenprofile, Templatepaket-Kandidaten, Parser, Writer, Kommunikationsservice und XDT-Baukasten-Geraeteausgabe-Vorschau sind vorhanden. Beim Ueberwachungsstart oeffnet kein RT-Fenster; erst eine stabile AIS-Datei oeffnet den Auswahl-/Sendedialog. Der Anwender sendet LM-/AR-Historienwerte bewusst ueber COM-Port, danach wartet XDTBox auf die spaetere Rueckgabe bis `EOT` plus Stabilitaetswartezeit und verarbeitet sie ueber das bestehende Mapping. Ein erster echter RT-3100-Praxismitschnitt ist als Hex-Fixture validiert: `SH Header CR`, `STX @RT CR`, zeilenweise STX-Daten, `EOT CR`, Final-R/L-SCA, ADD, VA, PD und WorkingDistance werden erkannt. Final-Prescription wird ueber `6228`, Subjective ueber `6227` abgebildet. Fuer Live-Abnahmen zeigt das RT-Fenster jetzt eine serielle Diagnose mit Profil-COM-Port, DTR/RTS/Handshake, RS-/SD-/Writer-Hexdump, sichtbaren Steuerzeichen, Empfang bis EOT und Timeout-Pruefliste; `COM-Port nur abhoeren` sendet nichts und erzeugt keinen Export. Der PC->RT-Livebefund zeigt, dass der direkte Writer-Frame am RT-3100 empfangen wurde; der Sendemodus ist deshalb jetzt im Schnittstellenprofil speicherbar und fuer RT-BuiltIns auf `DirectWriterFrame` voreingestellt. Nach erfolgreichem Senden haelt XDTBox den Pending-Workflow mit AIS-Kontext; `Rueckgabe abhoeren und verarbeiten` empfaengt die spaetere RT-Rueckgabe produktiv und exportiert erst dann MEDISTAR-XDT. MEDISTAR-Import steht praktisch noch aus.
- Der RT-3100-Livebefund vom 2026-05-29 konkretisiert die serielle Praxisabnahme: Mit DTR aus kam keine Rueckgabe, mit DTR aktiv/RTS aktiv wurde ein vollstaendiger Frame empfangen. Die RT-Type1-/Type2-Presets bleiben deshalb standardmaessig bei DTR/RTS aktiv. Die RS-Anforderung nutzt nach Auswertung produktiver RT-Anbindungen echte ASCII-Sternchen (`01 43 2A 2A 02 52 53 17 04`); LM-SCA-Augenpraefixe gehen weiterhin als Leerzeichen + `R`/`L` (`20 52`/`20 4C`) statt `2A 52`/`2A 4C` heraus. Fuer PC->RT stehen jetzt getrennte Frameformen bereit: `RT-Referenz ohne ID (AR/AL)` als BuiltIn-Default fuer RT-2100/RT-3100/RT-5100, die dokumentnahe Variante mit optionalem ID-Block, und die RT-3100-Praxisvariante `RT-3100 Praxisvariante (getestet)`, die den live angenommenen 107-Byte-Direct-Writer-Frame mit Legacy-ADD `RA`/`LA` und ohne zusaetzliches `EB` direkt vor `ET` exakt reproduziert. Speicherbare Sendeinhaltsvarianten (`RT-Referenz ohne ID (AR/AL)`, `RT-3100 Praxisvariante (getestet)`, `Alle ausgewaehlten Werte`, `Nur Autoref`, `Nur Lensmeter`, `Nur Lensmeter ohne ADD`, Varianten ohne ID, `Minimal rechts`) plus Testoption `CR nach EOT` sind vorhanden. Zwei weitere RT-3100-Nur-Abhoeren-Fixtures sichern den Befund ab, eines ohne ADD/VA und eines mit ADD/VA. Fuer PC->RT stehen im RT-Floating-Fenster explizite Sendetestmodi (`RS anfordern`, `DTR-Toggle + RS`, `Direkt Writer-Frame senden`, `RS + Writer ohne SD-Warten`) sowie CTS/DSR/DCD/RI-Modemstatusdiagnose bereit; diese Tests erzeugen keinen produktiven XDT-Export. Der live bestaetigte `DirectWriterFrame` ist zusaetzlich ein gespeicherter produktiver Schnittstellenprofilmodus: XDTBox sendet dann ohne RS/SD-Pflicht, meldet keinen SD-Fehler und wartet ohne leeren Export auf die spaetere Phoropter-Rueckgabe.
...
- weitere produktive RS232-Geraeteparser fuer serielle Altgeraete ausserhalb der NIDEK-RT-Phoropterfamilie; Shin-Nippon Accuref R-800/K-900, DL-1000/DL-800/DL-900, NCT-200 und SLM-4000 sind testseitig als serielle Text-BuiltIns vorbereitet, brauchen aber echte Praxisrohdateien. Fuer RT-3100 liegen echte Final-Prescription-Mitschnitte vor und der patientengetriggerte Sende-/Empfangsablauf ist implementiert. Empfang RT->XDTBox ist mit DTR aktiv bestaetigt; PC->RT-Senden per `DirectWriterFrame` wurde am RT-3100 empfangen und ist als Schnittstellenprofilmodus produktiv nutzbar. Die spaetere Rueckgabe kann im Wartestatus ueber `Rueckgabe abhoeren und verarbeiten` produktiv mit AIS-Kontext exportiert werden; MEDISTAR-Import muss weiterhin praktisch freigegeben werden
