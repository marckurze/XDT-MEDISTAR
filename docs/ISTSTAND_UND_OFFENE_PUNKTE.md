...
- grafische Geraetefenster-Standarddarstellung aus dem TOPCON-CV5000-Pilot: breitere Standardkarten und Floating-Fenster, groesseres Geraetebild oder Platzhalter, linker Infoblock mit kurzen Monitoring-Displaylabels fuer lange Geraetetypen, Text-ueber-Bild-Fallback fuer sehr lange Anzeigenamen, Eingangskacheln mit moderater Mitwachstoleranz fuer laengere Bezeichnungen, Statuskugel rot/statisch bei gestoppter Ueberwachung, gruen/pulsierend bei laufender Ueberwachung und schneller weiss/gelb/weiss-Blitz bei Dateieingang, linksbuendige Intervallsteuerung und `Letzter Scan`/`Automatik` im Detailbereich; leere Eingangskacheln zeigen bei laufender Ueberwachung `wartet` statt `gestoppt`. Neue UserDefined-Geraete koennen optional einen einfachen Geraetebildpfad speichern; bestehende Geraete koennen ueber `Geraet laden` ein lokales Bild bekommen, wobei BuiltIns nur lokale Bild-Overrides erhalten und fachlich unveraendert bleiben. `Geraet laden` und Monitoring nutzen dieselbe Bildaufloesung: Override, BuiltIn-App-Asset, danach Platzhalter. Alle offiziellen BuiltIn-Geraetebilder liegen als App-Assets unter `XdtDeviceBridge.App\Assets\Devices` und sind Bestandteil des nackten Kundeninstallers; fuer `Generisches Dokumentgeraet` und `Manuelle Dokumentauswahl` gibt es getrennte offizielle Assets. Lokale Overrides bleiben Kundendaten.
- Geraete-Steckbriefe sind fuer alle 65 aktuellen BuiltIn-Geraete vorbereitet. Die Originale liegen als App-Asset unter `XdtDeviceBridge.App\Assets\DeviceInfo\device-technical-profiles.de.json`, nutzen `deviceProfileId` als Zuordnung, referenzieren die offiziellen BuiltIn-Geraetebilder und beschreiben nur sicher ableitbare Geraete- und Messwertinformationen. Lokale Anpassungen und die `Notiz Techniker zum Geraet` werden in `%LocalAppData%\XdtDeviceBridge\device-info-overrides.json` gespeichert, ueber `DeviceTechnicalProfileService` mit dem Original zusammengefuehrt und durch Sicherung/Umzug gesichert beziehungsweise wiederhergestellt. Die grafische Steckbriefansicht ist jetzt aus `Schnittstellenprofile` und `XDT-Baukasten` erreichbar; die Mitnahme lokaler Steckbriefanpassungen beim Profil-/Templateexport bleibt ein spaeterer Abgrenzungspunkt.
- UI-Ausbau fuer Techniker ist umgesetzt: `Schnittstellenprofile` zeigt Geraetebild, `AIS Ausgabe Info`, `Geraetesteckbrief` und einen grossen Aktivierungsstatusbutton statt der alten Aktivierungscheckbox. Der XDT-Baukasten besitzt ebenfalls einen Steckbriefbutton in der Geraeteidentitaet. Der neue Tab `Notizen Techniker` speichert Whiteboard-Texte, Bilder, aktive Geraeteuebersicht und PDF-Export als Kundendaten; Sicherung/Umzug nimmt `technician-notes` mit.
- UI-Polish ist ueber ein zentrales XDTBox-Theme umgesetzt: `XdtBoxTheme.xaml` definiert Farben, Karten, moderne Buttonvarianten, Tab-Optik, Eingabefelder, Tabellen, Toggle-Schalter und dezente Schatten. MainWindow, AIS-Ausgabe-Info, Geraetesteckbrief, App-Einstellungen und Techniker-Notizen nutzen diese gemeinsamen Styles; Fachlogik, Parser, MEDISTAR-/XDT-Ausgabe, Geraetekommunikation, Lizenzlogik und Installer bleiben unveraendert. Nach Sichtpruefung wurden `Geraet laden`, `Schnittstellenprofile`, `Notizen Techniker` und `Lizenz` layoutseitig nachgeschaerft: Buttonzeilen sind getrennt, die unteren Schnittstellenprofil-Abschnitte ueberlappen nicht mehr, das Whiteboard nutzt Text-/Handmodus mit Kontextmenue, und Lizenz-Kundendaten sind in Praxis/Firma, Kontakt, Rechnungsversand und Bank/SEPA gegliedert.
- UI-Nachkorrektur fuer die finale 1.0-Oberflaeche: `Geraet laden` ist hoeher, damit Bildaktionen und separater Schliessen-Button sichtbar bleiben; `Schnittstellenprofile` trennt `Ordnerbereinigung`, `Archivierung` und `Pruefung vor Aktivierung` inklusive Status-, Aktions-, Hinweis- und Tabellenzeilen; `Notizen Techniker` nutzt kompakte Symbolmodi (`T` und Handmodus), direkte Texteingabe und Doppelklick-Bearbeitung; `Lizenz` ordnet SEPA-Zustimmung und Rechnung-ohne-SEPA nebeneinander, ohne Rechnungsversand oder Lizenzlogik zu veraendern.
- Kleine UI-Feinkorrektur: `AIS Ausgabe Info` und `Notizen Techniker` nutzen fuer die unteren Aktionsbuttons wieder den hellen Standardbutton; im Tab `Schnittstellenprofile` heissen Ordnerauswahlbuttons `Ordner waehlen` und sitzen direkt neben den jeweiligen Eingabefeldern. Fachlogik, Parser, XDT-/MEDISTAR-Ausgabe, Lizenzlogik und Installer-Datenpolitik bleiben unveraendert.
- Optionaler Tab-Schutz ist vorhanden: alle Tabs ausser `Verarbeitung` koennen lokal gesperrt werden. Die Konfiguration wird als Salt/Hash gespeichert und von Sicherung/Umzug gesichert. Der Lizenz-Tab speichert zusaetzlich Rechnungs-E-Mail, IBAN, BIC, Kontoinhaber, SEPA-Zustimmung und Rechnung-Option als Lizenz-Kundendaten.
- `XdtBox.LicenseManager` ist als internes Herstellerwerkzeug um Kundenverwaltung erweitert: Lizenzanfragen legen Kunden ueber Kundennummer oder InstallationId an beziehungsweise aktualisieren sie, Bank-/SEPA-/Rechnungsdaten werden lokal gespeichert, ein Netto-Einzelpreis pro Geraeteanbindung berechnet Kunden- und Gesamtkosten, Kundendetails zeigen vertikal getrennt Installationen, aktive lizenzierte Anbindungen und Lizenzhistorie, die Kundenliste kann als PDF exportiert werden, und einzelne Kunden koennen als eigenes grafisches Kunden-PDF mit Installationen, Geraetestandorten, aktiven Anbindungen, Historie und monatlichen Netto-Summen exportiert werden. LicenseManager-Sicherungen nehmen Kunden, Preis, Einstellungen und Historie ohne private Schluessel auf. Zwei oder mehr Kunden koennen per Checkbox markiert und ueber `Zusammenführen` in einen Zielkunden uebernommen werden; Stammdaten werden im Dialog feldweise gewaehlt, Installationen, aktive Anbindungen mit Geraetestandort und Lizenzhistorie bleiben erhalten.
- Aktueller XDTBox-Lizenzmanager-Stand: Der sichtbare Produktname lautet `XDTBox Lizenzmanager`, die eigene Version ist `1.11`, neue Lizenzen werden ohne Ablaufdatum erzeugt und als `unbefristet` angezeigt, Zahlungsart ist sauber als SEPA-Lastschrift oder Bankueberweisung modelliert, Rechnungs-E-Mail bleibt separat, Kunden koennen mehrere Installationen fuehren, lokale Stornos reduzieren aktive Installationen/Geraete/Kosten ohne Online-Sperrbehauptung, die Kundenliste zeigt die Gesamtsumme monatlicher Lizenzen, und `Ausgestellte Lizenzen` trennt reduzierte Historienliste von strukturierten Details. Gesamt-Kundenliste und Einzelkunde koennen als grafisches PDF exportiert werden; das Einzelkunden-PDF enthaelt auch Geraetestandorte, aktive Anbindungen, Historie und monatliche Netto-Summen. Einzelne lokale Historieneintraege koennen entfernt werden, ohne Lizenzdateien, private Schluessel oder Kundenstammdaten zu loeschen. Das eigene Setup wird ueber `scripts\build-xdtbox-licensemanager-installer.ps1` gebaut und packt keine Kunden-App, private Schluessel, Kundendaten, History-Dateien oder Backups. XDTBox Kunden-App steht bei `1.12`; Setup-Versionen duerfen auseinanderlaufen und werden nur auf ausdruecklichen Auftrag neu gebaut.
- `XdtBox.LicenseWeb` ist als interne Web-V1 fuer den Hersteller-Lizenzmanager hinzugekommen. Die Razor-Pages-App ist serverseitig geschuetzt, nutzt Cookie-Login, Datenroot ausserhalb von `wwwroot`, wiederverwendet die vorhandenen Kunden-/Historien-/Backup-/PDF-/Issuer-Bausteine und bietet Dashboard, Kunden, Details, Einzelkunden-PDF, Gesamt-PDF, Lizenzanfrage-Upload, Lizenzdatei-Download bei sicher konfiguriertem Private Key, Backup/Restore, Einstellungen, Produktivdiagnose und gezielte Einzelkunden-Migration. Die interne Web-Version steht bei `0.2.0-web-production-prep`; Production-Start verlangt einen Hash-Admin, unsichere DataRoot-/Private-Key-Pfade werden blockiert, Publish-Artefakte sollen keine Produktivsettings oder Keydateien enthalten. Sie ist nicht produktiv auf xdtbox.de freigegeben; die statische Website wurde nicht geaendert.
- Finaler Pruefstand fuer diesen UI-Ausbau vom 2026-06-06: `dotnet build XdtDeviceBridge.sln` erfolgreich mit 0 Warnungen und 0 Fehlern; `dotnet test XdtDeviceBridge.sln` mit 1984 erfolgreichen Tests, 0 fehlgeschlagen und 0 uebersprungen. `scripts\build-xdtbox-installer.ps1` erzeugte `artifacts\installer\XDTBox_Setup_1.0.exe` mit 79.314.259 Bytes; die Publish-App startete erfolgreich aus `artifacts\publish\XDTBox`.
- Schnittstellenprofile koennen im Tab `Schnittstellenprofile` jetzt als sichere V1 direkt angelegt werden: Der Dialog `Neues Schnittstellenprofil anlegen` waehlt Profilname, AIS-, Geraete- und Exportprofil, speichert ausschliesslich ein inaktives UserDefined-Schnittstellenprofil mit leeren Ordnern und startet keine Verarbeitung oder Ordneranlage. BuiltIns bleiben geschuetzt; die fachliche Konfiguration erfolgt danach im bestehenden Tab.
- Der Speichern-Button im Tab `Schnittstellenprofile` gibt nach erfolgreichem Speichern ein kurzes dezentes Erfolgsfeedback (`Gespeichert`) und kehrt automatisch in den Normalzustand zurueck. Fehler- und Validierungsfaelle zeigen kein positives Feedback.
- Im Tab `Schnittstellenprofile` koennen Ordnerpfade schlank vorbereitet werden: `Ordner Default` traegt Standardpfade unter `C:\XDTBox\<Geraetename>` ein, ohne Ordner zu erstellen, und `Ordner anlegen` erstellt nur die aktuell eingetragenen Pfade mit sichtbarer Erfolgs- oder Fehlermeldung. Automatische Aktivierung, Verarbeitung und Ordnerbereinigung bleiben davon unberuehrt.
...
- RS232-/COM-Port-Diagnose ist im Tab `Schnittstellenprofile` und im `XDT-Baukasten` vorhanden. Sie listet verfuegbare Ports, erlaubt Parameter wie Baudrate, Datenbits, Stoppbits, Paritaet, DTR/RTS und Flusskontrolle, liest Rohdaten zeitbegrenzt als Text und Hexdump und meldet fehlende/belegte Ports robust. Produktive serielle medizinische Parser bleiben bis zu echten Geraetedaten offen.
...
- NIDEK RT-2100 / RT-3100 / RT-5100 ist als gemeinsame serielle RS232-Phoropterfamilie vorbereitet und im produktiven Ablauf angebunden. Die PDF-Auswertung bestaetigt SH/SX/EB/ET/CR-Steuerzeichen, RT-2100 Type1 mit 2400 7E2 sowie RT-3100/RT-5100 Type1 2400 7E2 und Type2 9600 8O1. BuiltIn-Profile, MEDISTAR-Exportprofile, Schnittstellenprofile, Templatepaket-Kandidaten, Parser, Writer, Kommunikationsservice und XDT-Baukasten-Geraeteausgabe-Vorschau sind vorhanden. Beim Ueberwachungsstart oeffnet kein RT-Fenster; erst eine stabile AIS-Datei oeffnet den Auswahl-/Sendedialog im Vordergrund. Der produktive RT-3100-Button `An RT-3100 senden` nutzt automatisch den Praxispfad `RS senden, dann Writer ohne SD`: XDTBox sendet `RS`, wartet kurz, sendet den Writer-Frame ohne harte SD-Pflicht und wechselt danach mit AIS-Patientenkontext in den Wartestatus auf die spaetere Phoropter-Rueckgabe. `Nichts senden` haelt denselben Wartestatus ohne PC->RT-Sendung. Die normale RT-Geraetekachel ist pro Schnittstellenprofil single-instance und zeigt die frueheren Sendetestbuttons nicht mehr an; `COM-Port nur abhoeren` bleibt Diagnose ohne Export, `Rueckgabe abhoeren und verarbeiten` verarbeitet spaetere RT-Daten produktiv. Der vollstaendige RT-3100-Produktivlauf ist praktisch bestaetigt: AIS-Patientendatei, Auswahlfenster, `RS senden, dann Writer ohne SD`, spaetere Rueckgabe, MEDISTAR-XDT-Ausgabe und terminales Schliessen ohne erneutes Oeffnen bis zur naechsten AIS-Patientendatei. RT-2100 und RT-5100 nutzen denselben vorbereiteten Modus, bleiben aber geraetespezifisch live offen.
- Der RT-3100-Livebefund vom 2026-05-29 konkretisiert die serielle Praxisabnahme: Mit DTR aus kam keine Rueckgabe, mit DTR aktiv/RTS aktiv wurde ein vollstaendiger Frame empfangen. Die RT-Type1-/Type2-Presets bleiben deshalb standardmaessig bei DTR/RTS aktiv. Die RS-Anforderung nutzt nach Auswertung produktiver RT-Anbindungen echte ASCII-Sternchen (`01 43 2A 2A 02 52 53 17 04`); LM-SCA-Augenpraefixe gehen weiterhin als Leerzeichen + `R`/`L` (`20 52`/`20 4C`) statt `2A 52`/`2A 4C` heraus. Fuer PC->RT stehen getrennte Frameformen bereit: `RT-Referenz ohne ID (AR/AL)` als BuiltIn-Sendeinhalt, die dokumentnahe Variante mit optionalem ID-Block und die RT-3100-Praxisvariante `RT-3100 Praxisvariante (getestet)`. Speicherbare Sendeinhaltsvarianten plus Testoption `CR nach EOT` bleiben im Schnittstellenprofil fuer weitere Praxisabnahmen erhalten, werden aber nicht als normale Praxis-Testbuttons in der RT-Kachel angeboten. Der aktuelle RT-3100-Produktivpfad meldet statt SD-Fehlern `RT-3100: RS gesendet.`, `RT-3100: Writer-Frame ohne SD-Warten gesendet.` und `RT-3100: Warte auf Rueckgabe vom Phoropter.`.
- Fuer die Lizenzierung liegt mit `docs/LIZENZIERUNG_PRODUKTIVPLAN.md` ein konkreter Produktivplan vor: V1 lizenziert ausschliesslich die Anzahl aktiver Geraeteanbindungen (`MaxActiveDeviceConnections`); LAN/UNC, `SerialRs232`, Dokumentanhang, Profile/Templates und Testbereiche sind keine separaten Module. Signierte Offline-Lizenz, 7-Tage-Karenz nach Hardwareumzug beziehungsweise Lizenzproblem, ungueltige-Signatur-Blockade und zentrale Gates vor Aktivierung beziehungsweise Verarbeitungsstart sind vorbereitet/dokumentiert, aber noch nicht produktiv hart angeschlossen. Lizenzanfragen enthalten jetzt Kundendaten und dokumentierende Geraeteanbindungsnamen; `XdtBox.LicenseManager` ist als internes grafisches Herstellerwerkzeug fuer Request-Einlesung, Lizenzdateierzeugung, Historie und Folgelizenzen vorhanden.
...
- Der alte Tab `Profile & Templates` ist entfernt. Der `XDT-Baukasten` ist die eigenstaendige Arbeitsflaeche fuer Entwurf/Test/Vorschau und besitzt keine Legacy-Tab-Abhaengigkeit. Er haelt AIS-Profil, Geraeteprofil, Exportprofil-Arbeitskopie, Testdaten, Rohdatenanzeigen, vier Ergebnisansichten (`Roh-XDT`, `Ansicht im AIS`, `Geraeteausgabe`, `Diagnose`), Exportregel-Entwurf und Platzhalter getrennt von produktiven Schnittstellenprofilen. `Ansicht im AIS` zeigt nur fachlich sichtbare Karteikartenzeilen ohne Patientendatenblock, Feldnummern und technische Diagnose; diese Details liegen in `Roh-XDT` beziehungsweise `Diagnose`. Exportregeln koennen in der Arbeitskopie hinzugefuegt/geloescht werden; feste Notizen ohne `SourcePath`, Undo und Platzhalter-Beispielwerte sind vorbereitet. Zusaetzlich trennt der Baukasten die Regelrichtung `Export an AIS` von `Export an Geraet`: Bei bidirektionalen Geraeten wie CV-5000/CV-5000S und RT-6100 koennen DeviceOutput-Regeln als eigene Arbeitskopie angezeigt, bearbeitet und in der Geraeteausgabe-Vorschau beruecksichtigt werden, ohne produktive Geraeteordner zu beschreiben. UserDefined-bidirektionale Geraete koennen eigene DeviceOutput-Regeln anlegen. Die produktive Verarbeitung bleibt davon getrennt und darf weiterhin streng pruefen. `Baukasten-Template laden` nutzt lokale Arbeitskopien, `Konfiguration als Template speichern` legt ein UserDefined-Exportprofil plus lokale Baukasten-Template-Datei an, und `Template Paket importieren` fuehrt die Importvorschau und Uebernahme direkt im Baukasten aus. `Verarbeitung starten` ist dort ein Testlauf im Speicher; es wird keine produktive AIS-Datei geschrieben und keine Ueberwachung gestartet.
- Die Profilpflege liegt im Tab `Profilverwaltung`: globale Profiluebersicht, Umbenennen, Duplizieren, geschuetztes Loeschen, lokale Templateuebersicht, Template-/Paket-Export, Geraet laden/Bildpflege und BuiltIn-Reparatur. Neue AIS-, Geraete-, Export- und Schnittstellenprofile werden dort gestartet beziehungsweise in den passenden Zielbereich gefuehrt. BuiltIns bleiben geschuetzt und Originalprofile werden nicht ueberschrieben.
- Die XDT-Baukasten-Ausgabeansichten besitzen reine Orientierungs-Zeilennummern; diese Nummern sind nicht Teil von Roh-XDT/XML. Auch die Exportregel-Tabelle zeigt eine laufende Nummer. Die Auswahl einer Exportregel markiert die zugehoerige Ausgabezeile in `Roh-XDT`, `Ansicht im AIS` oder `Geraeteausgabe`; Regeln ohne erzeugte Ausgabe melden dies im Entwurfsstatus. Unterstuetzte DeviceOutput-Regelaenderungen fuer CV-5000/CV-5000S und RT-6100 aktualisieren die Geraeteausgabe-Vorschau direkt.
...
- Die Installations-/Update-Datenpolitik ist dokumentiert (`docs/INSTALLATION_UPDATE_DATENPOLITIK.md`) und als `XdtBoxInstallationDataPolicy` testseitig abgesichert. App-Dateien, BuiltIns und offizielle BuiltIn-Geraetebilder duerfen aktualisiert/repariert werden; Kundendaten unter `%LocalAppData%\XdtDeviceBridge`, UserDefined-Profile, Baukasten-Templates, lokale Geraetebilder, `device-image-overrides.json`, Lizenzdateien inklusive `license.xdtboxlic`, UI-Komfortdaten und Backups bleiben erhalten. Deinstallation folgt Variante B: Kundendaten bleiben standardmaessig erhalten und externe Praxisordner werden nie blind geloescht.
...
- Der Tab `Schnittstellenprofile` ist zur besseren Uebersicht in einklappbare Bereiche gegliedert und startet standardmaessig ausgeklappt; die Vorschau `Pruefung vor Aktivierung` trennt Statusinformationen und Aktionsbuttons layoutseitig, damit keine Textueberlagerungen entstehen.
...
- NIDEK NT530P / NT-530P als praktisch validierter Tonometrie-/Pachymetrie-Workflow mit echter UTF-16-XML-Fixture, `6220`-Pachymetrie, `6205`-Tonometrie, BuiltIn-Schnittstellenprofil, selektivem Templatepaket-Test und korrigiertem Nachlauf nach erfolgreichem Mehrfachanhang-Export; JPG-/Mehrfachanhang-Sonderfaelle bleiben separat offen
- NIDEK AR-1, AR-1S, AR-310A, LM-1800PD, NT-1, NT-1E, NT-1P, NT-510 und NT-530 als NIDEK-Referenzbatch-6-Varianten: eigene BuiltIn-Geraete-/Export-/Schnittstellenprofile, ModelName-Aliase, synthetische neutrale Fixtures und selektiver Templatepaket-Test; praktische Rohdateien und MEDISTAR-Abnahmen bleiben offen
...
- TOPCON CL-300PDL als testseitig vorbereitete CL-300-Familienvariante mit eigener BuiltIn-Schnittstellenprofilkette, `6228`-Lensmeterausgabe, synthetischer neutraler Fixture und Templatepaket-Test
...
- TOPCON RM-800 als testseitig vorbereitete REF-XML-Variante mit eigener BuiltIn-Schnittstellenprofilkette, `6228`-Autorefraktion, synthetischer neutraler Fixture und Templatepaket-Test; KM/KRT wird nicht geraten
...
- TOPCON TRK-3 Omnia als testseitig vorbereitete TRK-2P-Familienvariante mit eigener BuiltIn-Schnittstellenprofilkette, synthetischer neutraler REF/KM/TM/CCT-Fixture und Templatepaket-Test fuer `6228`, `6221`, `6220` und `6205`
...
- NIDEK RT-6100 als bidirektionaler LAN-/MEM-200-Phoropter-Kandidat: AIS-Historienparser und Auswahlfenster nutzen fuer den Geraete-Input konservativ nur `V0`/Lensmeter und `V1`/Autorefraktion; der Baukasten kann zusaetzlich echte NIDEK-LM-/ARK-XML-Quellen als `LM_Base`/`REF_Base` auswerten. `NidekRt6100InputXmlWriter` erzeugt dokumentnahes Ophthalmology-XML fuer den konfigurierten MEM-200-Zielordner `DIRECT_RT_xx\TXT`, und der Rueckgabeparser erkennt `NIDEK_RT`/`Measure Type="RT"`. Rueckgaben werden analog CV5000 getrennt exportiert: `Best` vollstaendig ueber `6228`, `Full` vollstaendig ueber `6227`, keine `6330`-Zeilen und keine kuenstliche Trennzeile. Das bereitgestellte OCR-XML ist als defekt diagnostiziert; positive Tests nutzen echte NIDEK-LM-/ARK-/RT-6100-XML-Fixtures.
...
- fertige, auslieferbare ZIP-Templatepakete fuer vorbereitete und validierte Geraeteprofile; fuer ARK1S, AR360, LM7, CL300, Solos, KR800S, KR-1, TRK2P, CT1P, CT800A und RT6100 sind Referenz-/Kandidatendokumentation und temporaer erzeugte Export-/Import-Tests vorhanden, offizielle ZIP-Artefakte folgen erst nach Release-Regel
...
- praktische RT-6100-/MEM-200-/MEDISTAR-Abnahme: erzeugte `RTImport_...xml` im konfigurierten `DIRECT_RT_xx\TXT`-Ordner am Geraet einlesen, die echte RT-6100-Rueckgabe mit `Best`/`Full` pruefen und den MEDISTAR-Import der daraus erzeugten `6228`-/`6227`-Rueckgabe validieren
- Rodenstock Phoromat 2000 Beta-Livevalidierung: echte RS232-Rueckgabeframes aufnehmen, PC->Geraet-Sendeframe am Geraet pruefen und MEDISTAR-Import fuer `6228`-FN sowie `6227`-WD praktisch abnehmen. VA bleibt bis dahin nur ein Baukasten-Platzhalter und wird nicht automatisch produktiv ausgegeben.
- weitere produktive RS232-Geraeteparser fuer serielle Altgeraete ausserhalb der NIDEK-RT-Phoropterfamilie; Shin-Nippon Accuref R-800/K-900, DL-1000/DL-800/DL-900, NCT-200 und SLM-4000 sind testseitig als serielle Text-BuiltIns vorbereitet, brauchen aber echte Praxisrohdateien. Fuer RT-3100 liegen echte Final-Prescription-Mitschnitte vor und der patientengetriggerte Sende-/Empfangsablauf ist praktisch bestaetigt. Empfang RT->XDTBox ist mit DTR aktiv bestaetigt; PC->RT nutzt im Produktivablauf den Praxispfad `RS senden, dann Writer ohne SD`, die spaetere Rueckgabe wird mit AIS-Kontext exportiert, und die MEDISTAR-XDT-Ausgabe wurde praktisch erzeugt. RT-2100 und RT-5100 sind analog vorbereitet und brauchen je eine eigene Live-Abnahme.
...
- Steckbrief-Folgeschritt: Die Oeffnen-Buttons sind in `Schnittstellenprofile` und `XDT-Baukasten` integriert. Offen bleibt nur die spaetere Entscheidung, ob lokale Steckbrief-Overrides beim Profil-/Templateexport gezielt fuer das betroffene Geraeteprofil mitgenommen werden sollen; Sicherung/Umzug unterstuetzt sie bereits.
...
- XDTBox 1.0 ist die erste offizielle Installer-Version mit Inno-Setup, self-contained Publish und Deinstallation Variante B. `artifacts\installer\XDTBox_Setup_1.0.exe` wurde mit Inno Setup 6.7.3 erzeugt; Codesignatur und praktische Admin-Testinstallation bleiben als manuelle Abnahmepunkte offen. Der Kundenpublish wird vor jedem Neubau frisch erzeugt und hart gegen lokale Entwicklungs-/Kundendaten validiert, damit nur App-Dateien und BuiltIn-Werksvorlagen ausgeliefert werden.
...
Der Tab `Profilverwaltung` ist die zentrale Flaeche fuer reine Profilpflege. Er zeigt AIS-, Geraete-, Export- und Schnittstellenprofile sowie lokale Templatepakete und Baukasten-Templates mit Suche, BuiltIn/UserDefined-Filter, Profiltypfilter, Details, Nutzungshinweisen und Wartung. UserDefined-Profile koennen dort umbenannt, dupliziert und geschuetzt geloescht werden; BuiltIns bleiben gesperrt und werden nur als UserDefined-Kopie dupliziert. Verwendete Profile und aktive Schnittstellenprofile blockieren blindes Loeschen. Der `XDT-Baukasten` und `Schnittstellenprofile` koppeln nicht an alte Tab-Controls.
...
Die automatische Verarbeitung startet ueber die App-Einstellung standardmaessig beim App-Start. Sie laeuft nur innerhalb der geoeffneten XDTBox-App bei laufender Ueberwachung; einen separaten Auto-Verarbeiten-Haken gibt es nicht mehr.

### Profilverwaltung und XDT-Baukasten

Die `Profilverwaltung` enthaelt:

- Profiluebersicht fuer BuiltIn- und UserDefined-Profile
- schlanke V1-Anlage fuer neue AIS- und Geraeteprofile als UserDefined; BuiltIns werden nicht ueberschrieben, es wird nichts automatisch aktiviert und keine Verarbeitung gestartet
- Exportprofil-Neuanlage als UserDefined-Kopie einer Vorlage mit Weiterbearbeitung im `XDT-Baukasten`
- sichere V1-Anlage neuer Schnittstellenprofile als inaktive UserDefined-Kombination aus AIS-, Geraete- und Exportprofil; Ordner, Nachlauf und automatische Verarbeitung werden anschliessend im Tab `Schnittstellenprofile` gepflegt
- sichere Wartung fuer UserDefined-Profile
- lokale Baukasten-Templates und Templatepakete mit Umbenennen/Export
- Importvalidierung, Konfliktanalyse, Importplan, Dry-Run, Importvorschau und sichere UserDefined-Uebernahme ueber den Baukasten-Importdialog

Der `XDT-Baukasten` kann:

- AIS-Testdatei laden
- Geraetedatei oder RS232-Rohdaten laden
- Messwerte, Platzhalter und Exportregeln pruefen
- Roh-XDT, AIS-Ansicht, Geraeteausgabe und Diagnose anzeigen
- Baukasten-Templates speichern/laden
- Templatepakete sicher als UserDefined-Kopie importieren

Der Baukasten ist nicht der gewuenschte Standardweg fuer Anwender. Standard soll ein fertiges Geraeteprofil plus fertiges Templatepaket sein; der Baukasten bleibt fuer Sonderfaelle, Tests, Vorschau und kundenspezifische Anpassungen. Exportprofile und BuiltIn-Profile werden durch Baukasten-Arbeitskopien nicht veraendert.
...
Die Schnittstellenprofil-Auswahl besitzt Filter nach `Geraetehersteller` und `AIS-System`; Filter und begrenztes Schnittstellenprofil-Dropdown stehen in einer kompakten Kopfzeile. Aktive Schnittstellenprofile zaehlen immer als Geraeteanbindung; der fruehere UI-Haken fuer eine separate Lizenzpflicht ist entfernt und Legacy-Profile mit deaktivierter Lizenzpflicht umgehen die Geraetezaehlung nicht mehr.
...
`AIS Ausgabe Info` zeigt im Tab `Schnittstellenprofile` fuer das aktuelle Profil eine reine AIS-Ausgabetabelle mit Feldkennung, Bedeutung, AIS-Relevanz, Karteikarten-Sichtbarkeit und Hinweis. Unterhalb der Tabelle wird die Standard-Zeilenbenennung fuer das MEDISTAR-XDT-Setup aus den aktiven Messwertregeln abgeleitet: `V0` Lensmeter, `V1` Autorefraktor, `V2` Phoropter, `V4` subjektive Refraktion, `V7` Keratometer, `V8` Biometrie, `Y` Tonometrie und `P` Pachymetrie. Diese Zeilenkennungen sind nur Techniker-Hilfe; XDTBox erzeugt weiterhin nur XDT-Feldkennungen und zeigt dort keine Beispielwerte. Reine Dokumentgeraete zeigen `6302`, `6303` und `6305` als Karteikarten-relevant; Messgeraete mit optionalem Anhang behalten diese Linkfelder als optionale Zusatzfelder. Die empfohlene Untersuchungsart priorisiert die Geraeteidentitaet vor optionalen Anhangsfaehigkeiten: Tonometer/Pachymeter bleiben `TONO`, reine Dokument-/AttachmentOnly-Profile bleiben `DOKU`. `8402` wird als `Untersuchungsart` erklaert, aus der AIS-Datei uebernommen und unveraendert wieder ausgegeben; empfohlene Untersuchungsart-Defaults fuer neue Profile sind unter anderem `LENS`, `PHORO`, `AUTO`, `KERA`, `KOMB`, `TONO`, `PACHY`, `ENDO`, `OCT`, `DOKU` und `MESS`. Das `6305`-Layout trennt Pfadtemplate und Ordnerbuttons in eigenen Grid-Spalten.
...
- NIDEK NT530P: echte XML-Fixture, Parserwerte, korrigierter mehrzeiliger `6205`-/`6220`-Export mit Headern, Templatepaket-Kandidat und praktische Tonometrie-/Pachymetrie-MEDISTAR-Abnahme vorhanden; JPG-/Mehrfachanhang-Sonderfaelle bleiben offen
- NIDEK Batch 6 XML-Varianten: AR-1, AR-1S, AR-310A, LM-1800PD, NT-1, NT-1E, NT-1P, NT-510 und NT-530 sind mit eigenen BuiltIns, Parseraliasen, synthetischen Fixtures und Sammel-Templatepaket-Doku vorbereitet; echte Praxisrohdateien und MEDISTAR-Abnahmen offen
...
- Canon/ZEISS/Visionix Batch 8: Canon RK-F2, Canon TX-20P, ZEISS VISULENS 550, ZEISS VISUPLAN 500, ZEISS VISUREF 100, ZEISS IOLMaster 700, Visionix Retinomax 5, Visionix VX 120 und Visionix VX 650 sind mit eigenen BuiltIns, Parsern, synthetischen neutralen Fixtures und Sammel-Templatepaket-Doku vorbereitet; REF/Lensmeter gehen nach `6228`, KM nach `6221`, Tonometrie nach `6205`, Pachymetrie/CCT nach `6220`, IOLMaster-VKT/AL nach `6227` und IOLMaster-R1/R2 nach `6228`; echte Praxisrohdateien und MEDISTAR-Abnahmen offen
- Bewusst nicht umgesetzt aus Batch 8: Canon OCT A-1 Xephilo und Visionix Optovue iVue 80/100 als OCT-/Bildworkflows
- NIDEK RT-6100: bidirektionaler LAN-/MEM-200-Phoropter-Kandidat mit PDF-basierter Protokollauswertung, echten LM-7P-/ARK-1s-/RT-6100-XML-Fixtures, RT-6100-Inputwriter fuer `LM_Base`/`REF_Base`, Rueckgabeparser fuer `Best` -> `6228` und `Full` -> `6227`, BuiltIns und Templatepaket-Kandidat testseitig vorhanden; offen bleiben praktische MEM-200-/RT-6100-Input-Abnahme und MEDISTAR-Abnahme
...
| Installer/Deployment | Installer 1.0 mit Neuinstallation, Update und Deinstallation Variante B ist vorbereitet. | `installer/XDTBox.iss`, `scripts/build-xdtbox-installer.ps1` und `docs/INSTALLER_BUILD_ANLEITUNG.md` sind vorhanden; Inno Setup 6 wird auf dem Build-PC benoetigt. Offizielle BuiltIn-Geraetebilder werden vor dem Publish als App-Assets geprueft und in den nackten Installer aufgenommen; lokale `DeviceImages` und `device-image-overrides.json` bleiben ausgeschlossen. | passt | Praktische Installation/Update/Deinstallation auf Windows-Testsystem abnehmen und BuiltIn-Bilder im nackten First-Run pruefen. |
...
| hoch | Fertige Geraeteprofile und Templatepakete | BuiltIn-Geraeteprofile und MEDISTAR-Exportprofile sind fuer die zentralen Geraete beziehungsweise Workflows vorhanden; praktisch validiert sind MEDISTAR + NIDEK ARK1S, die AR360-Auto-Refraktor-XDT-Rueckgabe, die LM7-Lensmeter-XDT-Rueckgabe, NIDEK NT530P Tonometrie/Pachymetrie, NIDEK RT-3100 RS232, TOPCON CL300, TOPCON KR800S, TOPCON TRK2P inklusive TM/CCT-only-Teilmessung und TOPCON CT1P. Solos, KR-1 und CT800A sind testseitig mit echten XML-Fixtures, korrigierter MEDISTAR-Ausgabe und selektivem Templatepaket-Kandidaten direkt nutzbar. Matrix, Referenz-/Kandidatendokumentation, selektiver Export, reproduzierbare Export-/Import-Tests und stabilisierter App-Preview-Pfad sind vorhanden. | Dauerhafte ZIP-Release-Artefakte, AR360-XDT-Anhangfall, weitere LM7-Prisma-/PD-Faelle, NT530P-JPG-/Mehrfachanhang-Sonderfaelle und die praktische Solos-/KR-1-/CT800A-Abnahme fehlen. Fuer CL300 bleibt die H/V-Prisma-Darstellung weiter zu beobachten; fuer KR800S/TRK2P bleiben weitere SBJ-/Funktionstestfaelle zu beobachten; fuer KR-1 fehlen echte KM/KRT-/periphere-KRT-Dateien; fuer CT1P fehlt eine echte beidseitige CorrectedIOP/CCT-Fixture. | Release-Regel fuer validierte Referenzpakete anwenden, LM7-Prisma-/PD-Beispielfaelle sammeln, NT530P-JPG-/Mehrfachanhang separat abnehmen, Solos/KR-1/CT800A praktisch abnehmen und CL300-/KR800S-/TRK2P-/CT1P-Praxisprotokolle als Referenz halten. | Anwender muessen sonst weiterhin den Baukasten nutzen. | BuiltIn-Profile, TemplatePackageExporter, Testdaten |
...
| hoch | Lizenzsignatur und Lizenz-Gates | `.xdtboxlic`-Import, RSA-PSS/SHA-256-Signaturpruefung, Public-Key-Provider, internes Issuer-Tool, V1-Policy-Bewertung und Legacy-Hinweis sind vorhanden. | Produktiver Public Key, sicherer privater Schluesselprozess, zentrale Start-/Aktivierungs-Gates und finale Blockadeanzeige fehlen. | Produktiven Public Key/KeyId festlegen und zentrales Gate als eigenen Folgeschritt bauen. | Ohne Gate wird weiterhin nur angezeigt/gewarnt; ohne produktiven Schluesselprozess koennen noch keine echten Kundenlizenzen ausgestellt werden. | Lizenzmodell, Supportprozess |
...
| mittel | NIDEK NT530P Sonderfaelle validieren | Echte UTF-16-XML-Fixture, Parserwerte, Header-/Einzelzeilen fuer `6205` Tonometrie und `6220` Pachymetrie, BuiltIn-Geraete-/Export-/Schnittstellenprofil, selektiver Templatepaket-Test, Mehrfachanhang-Linkfelder, korrigierter Nachlauf/Monitoring-Reset und praktische Tono/Pachy-MEDISTAR-Abnahme sind vorhanden. | Praktische MEDISTAR-Nachpruefung fuer den kompletten JPG-Mehrfachanhanglauf nach Nachlauf-Fix fehlt. | JPG-/Mehrfachanhang-Sonderlauf mit MEDISTAR durchfuehren und protokollieren. | Attachment-Erwartung oder Bildzuordnung koennte fachlich abweichen. | Testdaten, MEDISTAR-Anforderungen |
...
| mittel | NIDEK RT-6100 praktisch abnehmen | PDF-Auswertung, echte LM-7P-/ARK-1s-/RT-6100-XML-Fixtures, BuiltIn-Profile, `NidekRt6100InputXmlWriter`, RT-6100-Rueckgabeparser und selektiver Templatepaket-Test sind vorhanden. Das OCR-erzeugte Beispiel-XML ist als malformed diagnostiziert. | Praktischer Test am MEM-200/RT-6100 mit `DIRECT_RT_xx\TXT` und MEDISTAR-Import mit `6228`/`6227` fehlen. | Ausgabeordner im Bereich `Ausgabe an Geraet` auf den passenden MEM-200-Ordner setzen, eine RT-6100-Inputdatei erzeugen, am Geraet einlesen und die echte Rueckgabe in MEDISTAR importieren. | Ohne Geraetetests bleiben Ordnerkonventionen und Dateinamensakzeptanz Praxisannahmen aus dem PDF; die XML-Fachstruktur ist testseitig mit echten Dateien abgesichert. | RT-6100/MEM-200, Testpatient ohne Echtdaten, MEDISTAR |
...
- NIDEK NT530P als validierten Tono/Pachy-Workflow halten und JPG-/Mehrfachanhang-Sonderfaelle separat abnehmen; NIDEK Batch 6 XML-Varianten mit echten Praxisrohdateien gegen die vorbereiteten BuiltIns pruefen; TOPCON CT1P als validierten `6205`/`6220`-Referenzkandidaten halten; TOPCON KR-1 mit `6228` und TOPCON CT800A mit `6205` praktisch abnehmen; TOPCON CL300, KR800S und TRK2P als validierte Referenzkandidaten halten.
...
- NT530P-JPG-/Mehrfachanhang-Sonderfaelle
