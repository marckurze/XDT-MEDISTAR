...
- stabilisierte manuelle Grundlage fuer abdockbare Geraeteanbindungsfenster im Tab `Verarbeitung`: Abdocken/Andocken ueber Symbolbutton `🗗`, Pin `🔝`, Positionsmerken `📌`, sicheres X-Schliessen, Start-Wiederherstellung nach sicherer MainWindow-Anzeige, Radar und `-`/`+` pro Schnittstellenprofil
...
| mittel | Abdockbare Geraeteanbindungsfenster | Manuelles Abdocken/Andocken per `🗗`, sicheres X-Schliessen, Pin `🔝`, Positionsmerken `📌`, verzoegerte Neustart-Wiederherstellung, Radar und `-`/`+` sind als UI-Grundlage vorhanden. | Systray, Auto-Popup bei Dateieingang, Auto-Zurueckdocken und Rueckdock-Zeit fehlen bewusst. | Praktisch mit AR360/ARK1S testen und danach Auto-Popup separat planen. | Ohne Auto-Popup muss der Benutzer noch manuell abdocken. | Verarbeitungstab, Monitoring-Karten |
