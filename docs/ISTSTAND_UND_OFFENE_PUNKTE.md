...
| Abdockbare Geraeteanbindungsfenster | UI-Grundlage testseitig fuer State, Persistenz, verzoegerte Wiederherstellung, Auto-Abdocken, Auto-Zurueckandocken, akustisches Signal nur bei Geraetedatei-Eingang, Systray-Fensterzustand und sicheren Vorgangsreset abgesichert | `InterfaceProfileFloatingWindowStateServiceTests`, `InterfaceProfileFloatingWindowStateRepositoryTests`, `InterfaceProfileFloatingWindowRestoreGateTests`, `InterfaceProfileAutoDetachServiceTests`, `InterfaceProfileAutoRedockServiceTests`, `InterfaceProfileNotificationSoundServiceTests`, `TrayWindowStateServiceTests`, `InterfaceProfileMonitoringResetServiceTests` |
...
| mittel | Abdockbare Geraeteanbindungsfenster | Manuelles Abdocken/Andocken per transparentem `🗗`, Vorgangsreset `↺`, sicheres X-Schliessen, Pin/TopMost `🔝` pro Floating-Fenster, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, verzoegerte Neustart-Wiederherstellung, Radar, automatische Oeffnung, akustisches Signal nur bei Geraetedatei-Eingang, automatisches Zurueckandocken 5 Sekunden nach terminalem Abschluss und Systray-Grundfunktion sind vorhanden. | Autostart, Windows-Dienst, UI-Einstellung fuer Rueckdock-Zeit, sichtbarer Countdown-Hinweis und UI-Schalter fuer Signalton fehlen bewusst. | Praktisch mit AR360/ARK1S testen: Reset bei falschem Vorgang, Tray minimieren, Floating-Fenster bei Aktivitaet, Ton, Auto-Zurueckandocken und echtes Beenden ueber Tray-Menue. | Reset ist bewusst sicher: keine Ordnerleerung, keine unbekannten Dateien; bekannte aktuelle Dateien werden in-memory ignoriert, bis sie geaendert oder entfernt werden. | Verarbeitungstab, Monitoring-Karten |
