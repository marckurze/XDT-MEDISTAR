...
- stabilisierte manuelle Grundlage fuer abdockbare Geraeteanbindungsfenster im Tab `Verarbeitung`: Abdocken/Andocken ueber transparenten Symbolbutton `🗗`, Pin `🔝`, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, sicheres X-Schliessen, Start-Wiederherstellung nach sicherer MainWindow-Anzeige und Radar pro Schnittstellenprofil
...
- Systray-Betrieb, automatisches Abdocken bei Dateiempfang und automatisches Zurueckdocken fuer abdockbare Geraeteanbindungen
...
| Abdockbare Geraeteanbindungsfenster | UI-Grundlage testseitig fuer State, Persistenz und verzoegerte Wiederherstellung abgesichert | `InterfaceProfileFloatingWindowStateServiceTests`, `InterfaceProfileFloatingWindowStateRepositoryTests`, `InterfaceProfileFloatingWindowRestoreGateTests` |
...
| mittel | Abdockbare Geraeteanbindungsfenster | Manuelles Abdocken/Andocken per transparentem `🗗`, sicheres X-Schliessen, Pin `🔝`, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, verzoegerte Neustart-Wiederherstellung und Radar sind als UI-Grundlage vorhanden. | Systray, Auto-Popup bei Dateieingang, Auto-Zurueckdocken und Rueckdock-Zeit fehlen bewusst. | Praktisch mit AR360/ARK1S testen und danach Auto-Popup separat planen. | Ohne Auto-Popup muss der Benutzer noch manuell abdocken. | Verarbeitungstab, Monitoring-Karten |
