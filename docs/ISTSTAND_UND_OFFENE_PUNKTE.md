...
- Aktivierungsassistent fuer importierte Schnittstellenprofile; erste read-only Backend-Bewertung und eine UI-Pruefvorschau im Tab `Schnittstellenprofile` sind vorhanden, inklusive strukturierter Ordner- und XDT-Anhang-Detailanzeige; produktive Aktivierung bleibt offen
...
| hoch | Templatepaket-Import Aktivierungsassistent | Sicherer Import als UserDefined vorhanden; Backend-Bewertung und UI-Pruefvorschau fuer importierte Schnittstellenprofile pruefen Abhaengigkeiten, Ordner, XDT-Anhang-Konfiguration und Lizenzhinweise read-only. Ordner und XDT-Anhang-Einstellungen werden strukturiert angezeigt. | Bewusste Benutzerfreigabe und eigentliche Aktivierung fehlen weiterhin. | Naechsten kleinen Schritt als gefuehrte Aktivierungsvorbereitung planen, weiterhin ohne automatische Aktivierung. | Importierte Profile bleiben zwar sicher, aber fuer Anwender noch nicht komfortabel produktiv nutzbar. | TemplateImport Executor, Profilkatalog, InterfaceProfileActivationEvaluationService |
