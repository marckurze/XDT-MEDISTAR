...
- NIDEK RT-6100 als bidirektionaler LAN-/MEM-200-Phoropter-Kandidat: AIS-Historienparser und Auswahlfenster nutzen fuer den Geraete-Input konservativ nur `V0`/Lensmeter und `V1`/Autorefraktion, `NidekRt6100InputXmlWriter` erzeugt PDF-nahe `LM_Base`-/`REF_Base`-Ophthalmology-XML fuer den konfigurierten MEM-200-Zielordner `DIRECT_RT_xx\TXT`, und der Rueckgabeparser erkennt `NIDEK_RT`/`Measure Type="RT"`. Rueckgaben werden analog CV5000 getrennt exportiert: `Best` vollstaendig ueber `6228`, `Full` vollstaendig ueber `6227`, keine `6330`-Zeilen und keine kuenstliche Trennzeile. Das bereitgestellte OCR-XML ist als defekt diagnostiziert; positive Tests nutzen PDF-nahe wohlgeformte Fixtures.
...
- NIDEK RT-6100: bidirektionaler LAN-/MEM-200-Phoropter-Kandidat mit PDF-basierter Protokollauswertung, RT-6100-Inputwriter fuer `LM_Base`/`REF_Base`, Rueckgabeparser fuer `Best` -> `6228` und `Full` -> `6227`, BuiltIns und Templatepaket-Kandidat testseitig vorhanden; offen bleiben praktische MEM-200-/RT-6100-Input-Abnahme und MEDISTAR-Abnahme echter Rueckgabedateien
...
| mittel | NIDEK RT-6100 praktisch abnehmen | PDF-Auswertung, BuiltIn-Profile, `NidekRt6100InputXmlWriter`, RT-6100-Rueckgabeparser und selektiver Templatepaket-Test sind vorhanden. Das OCR-erzeugte Beispiel-XML ist als malformed diagnostiziert. | Praktischer Test am MEM-200/RT-6100 mit `DIRECT_RT_xx\TXT`, echte wohlgeformte Rueckgabedatei und MEDISTAR-Import mit `6228`/`6227` fehlen. | Ausgabeordner im Bereich `Ausgabe an Geraet` auf den passenden MEM-200-Ordner setzen, eine RT-6100-Inputdatei erzeugen, am Geraet einlesen und die echte Rueckgabe in MEDISTAR importieren. | Ohne Geraetetests bleiben Ordnerkonventionen, Dateinamensakzeptanz und echte Rueckgabedetails Praxisannahmen aus dem PDF. | RT-6100/MEM-200, Testpatient ohne Echtdaten, MEDISTAR |
