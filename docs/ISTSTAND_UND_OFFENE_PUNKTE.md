...
Die offenen fachlichen Entscheidungen sind in `docs/AKTIVIERUNG_ENTSCHEIDUNGSNOTIZ.md` gebuendelt. Die Notiz enthaelt inzwischen eine vorlaeufige V1-Entscheidungslinie und eine konkretere V1-Spezifikation: Aktivierung nur fuer UserDefined, BuiltIn bleibt geschuetzt, `Ready` ist aktivierbar, `ReadyWithWarnings` nur nach bewusster Bestaetigung, `Blocked`/`Unknown` nie aktivieren, `IsAttachmentProcessingEnabled` bleibt separat und finale Re-Evaluation direkt vor Speicherung ist Pflicht. Der Aktivierungsstatus wird fachlich dem UserDefined-Schnittstellenprofil zugeordnet; `IsActive` ist der naheliegende Kandidat, muss aber vor Implementierung endgueltig bestaetigt werden. Die Persistenz soll an der bestehenden UserDefined-Schnittstellenprofil-Ablage im Profilkatalog ansetzen, nicht an AIS-, Geraete- oder Exportprofilen. Ergaenzend beschreibt die Notiz eine V1-Linie fuer spaetere Deaktivierung: Deaktivierung ist keine Loeschung, keine Ordnerbereinigung, keine Dateioperation, keine Aenderung an `IsAttachmentProcessingEnabled` und betrifft nur UserDefined-Schnittstellenprofile. Fuer laufende und wartende Pakete ist als Leitplanke dokumentiert: Aktivierung startet keine Sofortverarbeitung, Deaktivierung verhindert neue Paketstarts, bestehende Pakete duerfen nicht stillschweigend verworfen werden und die konservativste V1-Option ist zu pruefen, Deaktivierung bei aktivem oder wartendem Paket zu blockieren. Diese Linien sind weiterhin nur Konzept/Dokumentation und keine Freigabe fuer produktive Aktivierung oder Deaktivierung.
...
Offen bleiben die echte produktive Aktivierung und Deaktivierung, die finale technische Bestaetigung des Aktivierungsflags, die konkrete Repository-/Store-Methode, eine bewusste Warnungsbestaetigungs-UI, eine moegliche auditierbare Speicherung einer Warnungsbestaetigung, eine produktive `ActivationExecutor`-Implementierung beziehungsweise ein Deaktivierungspfad, Audit-Speicherort, Benutzerrollen, Lizenzdurchsetzung, Verhalten bei parallelen Aenderungen, konkrete Paketstatus-Erkennung, Umgang mit Altbestand/Fingerprint, Umgang mit laufenden/wartenden Paketen, Reaktivierungsdetails und die finale fachliche Freigabe der in `docs/AKTIVIERUNG_ENTSCHEIDUNGSNOTIZ.md` dokumentierten V1-Linie.
...
- V1-Spezifikation aus `docs/AKTIVIERUNG_ENTSCHEIDUNGSNOTIZ.md` fachlich final abnehmen: Aktivierungsflag, UserDefined-Persistenz, Aenderungsschutz, Warnungsbestaetigung/Audit, Deaktivierungslinie, Paketregel fuer laufende/wartende Pakete und finale Re-Evaluation.
- UI-Konzept fuer spaetere bewusste Warnungsbestaetigung ohne sofortige Speicherung entwerfen.
- Produktive `ActivationExecutor`-Implementierung und Deaktivierungspfad erst nach Fachentscheidung zu Preconditions, Audit, Rollen, Speicherung, Warnungsbestaetigung, Deaktivierungsregeln, Paketstatus-Erkennung und konkreter Store-Methode planen; Entscheidungsgrundlage: `docs/AKTIVIERUNG_ENTSCHEIDUNGSNOTIZ.md`.
