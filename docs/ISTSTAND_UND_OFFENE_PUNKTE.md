...
- Der neue Tab `XDT-Baukasten` ist als eigenstaendige Nachfolge-Arbeitsflaeche fuer den bisherigen Sammelbereich angelegt. Er haelt AIS-Profil, Geraeteprofil, Exportprofil-Arbeitskopie, Testdaten, Rohdatenanzeigen, vier Ergebnisansichten (`Roh-XDT`, `Ansicht im AIS`, `Geraeteausgabe`, `Diagnose`), Exportregel-Entwurf und Platzhalter getrennt von produktiven Schnittstellenprofilen. `Ansicht im AIS` zeigt nur fachlich sichtbare Karteikartenzeilen ohne Patientendatenblock, Feldnummern und technische Diagnose; diese Details liegen in `Roh-XDT` beziehungsweise `Diagnose`. Exportregeln koennen in der Arbeitskopie hinzugefuegt/geloescht werden; feste Notizen ohne `SourcePath`, Undo und Platzhalter-Beispielwerte sind vorbereitet. Zusaetzlich trennt der Baukasten die Regelrichtung `Export an AIS` von `Export an Geraet`: Bei bidirektionalen Geraeten wie CV-5000/CV-5000S und RT-6100 koennen DeviceOutput-Regeln als eigene Arbeitskopie angezeigt, bearbeitet und in der Geraeteausgabe-Vorschau beruecksichtigt werden, ohne produktive Geraeteordner zu beschreiben. UserDefined-bidirektionale Geraete koennen eigene DeviceOutput-Regeln anlegen. Geraeteplatzhalter trennen Definition und aktuellen Messwert und koennen im Baukasten bewusst Werte aus einer geladenen, parsebaren Testdatei anzeigen, auch wenn das gewaehlte Profil abweicht. Die Baukasten-Kompatibilitaetspruefung ist zentralisiert: BuiltIn-Profile pruefen bekannte Modell-Aliase wie `CV-5000`/`CV-5000S`, ModelName-/Profilabweichungen blockieren den Baukasten aber nicht, sondern erscheinen als Warnung und Diagnosehinweis. UserDefined-/Entwurfsprofile bleiben fuer neue Geraete offen und koennen geparste Messpfade dynamisch anzeigen. Die produktive Verarbeitung bleibt davon getrennt und darf weiterhin streng pruefen. `Baukasten-Template laden` nutzt lokale Arbeitskopien, und `Template Paket importieren` fuehrt die Importvorschau und Uebernahme direkt im Baukasten aus. `Verarbeitung starten` ist dort ein Testlauf im Speicher; es wird keine produktive AIS-Datei geschrieben und keine Ueberwachung gestartet. Der alte Tab `Profile & Templates` bleibt bis zur vollstaendigen Abloesung parallel vorhanden.
- Der XDT-Baukasten ist nun weiter vom alten Tab `Profile & Templates` entkoppelt: Die App initialisiert Legacy-Profilverwaltung und Baukasten/Schnittstellenprofile getrennt. `Baukasten-Template laden` nutzt lokale `.xdtbaukasten.template.json`-Arbeitskopien, `Konfiguration als Template speichern` legt ein UserDefined-Exportprofil plus lokale Baukasten-Template-Datei an, und `Template Paket importieren` besitzt einen eigenen Baukasten-Importdialog mit Vorschau, Konfliktentscheidung und UserDefined-Uebernahme. Das Baukasten-RS232-Fenster bietet DTR/RTS, Rohtext/Hexdump und einen kleinen ASCII-Sendetest. Die wichtigsten reinen Profilpflegefunktionen sind in den neuen Tab `Profilverwaltung` gewandert: globale Profiluebersicht, Umbenennen, Duplizieren, geschuetztes Loeschen, lokale Templateuebersicht und BuiltIn-Reparatur. Offen bleiben praktische UI-Abnahme, moegliche Zentralisierung der Geraetebildpflege und einzelne Legacy-Diagnosepfade vor Entfernung von `Profile & Templates`.
...
Die App hat aktuell sieben Haupt-Tabs:
...
Der neue Tab `Profilverwaltung` ist die zentrale Uebergangsflaeche fuer reine Profilpflege. Er zeigt AIS-, Geraete-, Export- und Schnittstellenprofile sowie lokale Templatepakete und Baukasten-Templates mit Suche, BuiltIn/UserDefined-Filter, Profiltypfilter, Details, Nutzungshinweisen und Wartung. UserDefined-Profile koennen dort umbenannt, dupliziert und geschuetzt geloescht werden; BuiltIns bleiben gesperrt und werden nur als UserDefined-Kopie dupliziert. Verwendete Profile und aktive Schnittstellenprofile blockieren blindes Loeschen. Damit ist die spaetere Entfernung von `Profile & Templates` vorbereitet, ohne `XDT-Baukasten` oder `Schnittstellenprofile` an alte Tab-Controls zu koppeln.
