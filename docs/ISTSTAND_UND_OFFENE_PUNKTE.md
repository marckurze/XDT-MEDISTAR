...
`AIS Ausgabe Info` zeigt im Tab `Schnittstellenprofile` fuer das aktuelle Profil eine reine AIS-Ausgabetabelle mit Feldkennung, Bedeutung, AIS-Relevanz, Karteikarten-Sichtbarkeit und Hinweis. Reine Dokumentgeraete zeigen `6302`, `6303` und `6305` als Karteikarten-relevant; Messgeraete mit optionalem Anhang behalten diese Linkfelder als optionale Zusatzfelder. Die empfohlene Untersuchungsart priorisiert die Geraeteidentitaet vor optionalen Anhangsfaehigkeiten: Tonometer/Pachymeter bleiben `TONO`, reine Dokument-/AttachmentOnly-Profile bleiben `DOKU`. `8402` wird als `Untersuchungsart` erklaert, aus der AIS-Datei uebernommen und unveraendert wieder ausgegeben; empfohlene Untersuchungsart-Defaults fuer neue Profile sind unter anderem `LENS`, `PHORO`, `AUTO`, `KERA`, `KOMB`, `TONO`, `PACHY`, `ENDO`, `OCT`, `DOKU` und `MESS`. Das `6305`-Layout trennt Pfadtemplate und Ordnerbuttons in eigenen Grid-Spalten.
...
- Canon/ZEISS/Visionix Batch 8: Canon RK-F2, Canon TX-20P, ZEISS VISULENS 550, ZEISS VISUPLAN 500, ZEISS VISUREF 100, Visionix Retinomax 5, Visionix VX 120 und Visionix VX 650 sind mit eigenen BuiltIns, `CanonZeissVisionixDeviceParser`, synthetischen neutralen Fixtures und Sammel-Templatepaket-Doku vorbereitet; REF/Lensmeter gehen nach `6228`, KM nach `6221`, Tonometrie nach `6205`, Pachymetrie/CCT nach `6220`; echte Praxisrohdateien und MEDISTAR-Abnahmen offen
- Bewusst nicht umgesetzt aus Batch 8: Canon OCT A-1 Xephilo und Visionix Optovue iVue 80/100 als OCT-/Bildworkflows; ZEISS IOLMaster 700 bleibt ohne klare MEDISTAR-Ziel-Feldkennung Kandidat
