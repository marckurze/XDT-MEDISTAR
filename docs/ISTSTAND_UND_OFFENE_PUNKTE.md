...
- stabilisierte Grundlage fuer abdockbare Geraeteanbindungsfenster im Tab `Verarbeitung`: Abdocken/Andocken ueber transparenten Symbolbutton `🗗`, Pin/TopMost `🔝` pro Floating-Fenster, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, sicheres X-Schliessen, Start-Wiederherstellung nach sicherer MainWindow-Anzeige, Radar pro Schnittstellenprofil, automatische Oeffnung bei Monitoring-Aktivitaet, akustisches Signal nur bei Geraetedatei-Eingang und automatisches Zurueckandocken nach terminalem Abschluss
...
- Systray-Betrieb, UI-Einstellung fuer die Rueckdock-Zeit und sichtbarer Countdown-Hinweis fuer abdockbare Geraeteanbindungen
...
| mittel | Abdockbare Geraeteanbindungsfenster | Manuelles Abdocken/Andocken per transparentem `🗗`, sicheres X-Schliessen, Pin/TopMost `🔝` pro Floating-Fenster, Positionsmerken `📌`, dezente `-`/`+`-Intervallbuttons, verzoegerte Neustart-Wiederherstellung, Radar, automatische Oeffnung, akustisches Signal nur bei Geraetedatei-Eingang und automatisches Zurueckandocken 5 Sekunden nach terminalem Abschluss sind vorhanden. | Systray, UI-Einstellung fuer Rueckdock-Zeit, sichtbarer Countdown-Hinweis und UI-Schalter fuer Signalton fehlen bewusst. | Praktisch mit AR360/ARK1S testen und danach Systray-Verhalten separat planen. | Terminaler Abschluss wird aus bestehenden Monitoring-Events abgeleitet; bei unklaren Statusmeldungen bleibt das Fenster konservativ offen. | Verarbeitungstab, Monitoring-Karten |
