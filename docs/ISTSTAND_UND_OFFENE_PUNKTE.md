...
- generischer AttachmentOnly-/Dokumentgeraete-V1-Kandidat `MEDISTAR + Dokumentanhang`: AIS-Datei plus stabile Dokumentdateien erzeugen optional eine oder mehrere `6227`-Zeilen und je Datei eigene `6302`-`6305`-Linkfelder, ohne Messwertparser und ohne `6228`/`6205`/`6220`; mehrere Dateien werden erst nach profilbezogener Ruhezeit oder nach manueller Bestaetigung uebertragen.
- Die Schnittstellenprofil-Konfiguration fuer AttachmentOnly ist kontextsensitiv reduziert: Der Geraete-/Dokument-Importordner ist der Dateieingang, separate XDT-Anhang-Importfelder werden ausgeblendet, und der manuelle Dialog sammelt weitere stabile Dateien bis zum Klick auf `Uebertragen`.

Vorbereitet, aber noch nicht als produktiv abgenommen:

- V2-Geraeteprofile fuer TOPCON CL300, TOPCON KR800 und TOPCON TRK2P
...
| hoch | Fertige Geraeteprofile und Templatepakete | BuiltIn-Geraeteprofile und MEDISTAR-Exportprofile sind fuer sieben Geraete vorhanden; praktisch validiert sind MEDISTAR + NIDEK ARK1S, die AR360-Auto-Refraktor-XDT-Rueckgabe und die LM7-Lensmeter-XDT-Rueckgabe. NT530P ist testseitig mit echter XML-Fixture, korrigiertem mehrzeiligem `6205`-/`6220`-Export und selektivem Templatepaket-Kandidaten direkt nutzbar. Matrix, Referenz-/Kandidatendokumentation, selektiver Export, reproduzierbare Export-/Import-Tests und stabilisierter App-Preview-Pfad sind vorhanden. | Dauerhafte ARK1S-/AR360-/LM7-ZIP-Release-Artefakte, AR360-XDT-Anhangfall und weitere LM7-Prisma-/PD-Faelle fehlen; fuer NT530P fehlt die praktische MEDISTAR-Abnahme inklusive optionalem JPG-Mehrfachanhangfall, fuer TOPCON fehlen Repository-Testdaten/Praxisabnahmen. | Release-Regel fuer ARK1S/AR360 anwenden, LM7-Prisma-/PD-Beispielfaelle sammeln und NT530P in MEDISTAR praktisch mit `6220`/`6205` sowie JPG-Mehrfachanhaengen abnehmen. | Anwender muessen sonst weiterhin den Baukasten nutzen. | BuiltIn-Profile, TemplatePackageExporter, Testdaten |
...
| mittel | Dokumentgeraete produktiv validieren | BuiltIn `MEDISTAR + Dokumentanhang`, AttachmentOnly-Modus, kontextsensitive Konfiguration, mehrzeiliger `6227`-Dokumentationstext, mehrere `6302`-`6305`-Linkfeldgruppen, Sammeln bis `Uebertragen` und selektiver Templatepaket-Test sind vorhanden. | Praktischer MEDISTAR-Test mit PDF/JPG/XML/Medienanhaengen, mehrzeiligem optionalem Text und realistischen Copy/Move-Zielordnern fehlt. | Live-Test mit mehreren stabilen Dokumentdateien protokollieren; dabei Ruhezeit- und manuellen Bestaetigungsmodus pruefen und danach entscheiden, ob Drag-&-Drop oder Vorschaukacheln separat gebraucht werden. | Dokumentgeraete bleiben ohne Praxisabnahme ein technischer Kandidat. | Testanhaenge, MEDISTAR-Anforderungen |
...
- NIDEK NT530P als naechsten praktischen MEDISTAR-Testkandidaten mit korrigiertem `6205`/`6220`-Layout und optionalem JPG-Mehrfachanhang abnehmen; TOPCON-Profile erst nach Datenlage priorisieren.
- Dokumentgeraete-V1 mit AIS-Datei, mehreren Anhaengen und optionalem `6227`-Text praktisch in MEDISTAR pruefen.
