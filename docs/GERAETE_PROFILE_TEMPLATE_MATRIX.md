...
- Die Templatepaket-Infrastruktur ist vorhanden und testseitig abgesichert. Fuer MEDISTAR + NIDEK ARK1S gibt es jetzt die offizielle Paketvorlage `docs/TEMPLATEPAKET_MEDISTAR_NIDEK_ARK1S.md`; die reproduzierbar erzeugte ZIP-Paketdatei bleibt der naechste kleine Schritt.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| NIDEK | ARK1S | Autorefraktor | produktiv validiert | `XmlDeviceParser`, ARK1S-Testdatei | BuiltIn `device-nidek-ark1s-default` plus altes Standardprofil | Core-/Pipeline-, DeviceProfile-, ExportProfile-, TemplatePackage-Tests | offizielle Vorlage vorhanden; ZIP-Artefakt noch offen | MEDISTAR | validiert fuer externen Link | Vorlage mit `TemplatePackageExporter` reproduzierbar als Paketdatei erzeugen | Nicht durch neue Profil-/Templatearbeit regressieren |
...
1. MEDISTAR + NIDEK ARK1S sauber halten: keine Regression am validierten Kernworkflow, offizielle Paketvorlage in eine reproduzierbar erzeugte ZIP-Paketdatei ueberfuehren.
...
- Fertige Templatepaket-Dateien fehlen noch; fuer ARK1S ist die Vorlage dokumentiert.
