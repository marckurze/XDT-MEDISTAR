...
| MEDISTAR + Dokumentanhang | MEDISTAR | `device-document-attachment-default` | `export-medistar-document-attachment-default` | `interface-medistar-document-attachment-default` | ja, `docs/TEMPLATEPAKET_MEDISTAR_DOKUMENTANHANG.md` | ja, `MedistarDocumentAttachmentTemplatePackageTests` | nein, noch offen | testseitig ueber mehrere `6302`-`6305`-Gruppen | nein | Live-Test mit AIS-Datei, optionalem `6227`-Text und mehreren Anhangdateien durchfuehren |
...
| Generisch | Dokumentgeraet | Dokument-/Dateianhang ohne Messwerte | V1-Kandidat, testseitig direkt nutzbar | kein Messwertparser; AttachmentOnly verarbeitet stabile Dateien als Anhaenge | BuiltIn `device-document-attachment-default` plus BuiltIn-Schnittstellenprofil | ImportFileClassifier-, AttachmentScanner-, ManualProcessor-, AutoImportScanner- und TemplatePackage-Tests | Kandidat dokumentiert; selektiver Export/Import reproduzierbar testgeprueft; ZIP-Release-Artefakt noch offen | MEDISTAR | ja, mehrere Anhaenge je Vorgang ueber `6302`-`6305`; optional `6227` Dokumentationstext | Praktischen MEDISTAR-Live-Test mit PDF/JPG/XML/Medienanhaengen und optionalem Text durchfuehren | Keine Vorschau/Drag-&-Drop/OCR; XML wird in diesem Modus nicht als Messwert geparst |
