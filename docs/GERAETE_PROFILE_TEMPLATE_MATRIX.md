...
- Fertige BuiltIn-Schnittstellenprofile existieren aktuell fuer `MEDISTAR + NIDEK ARK1S`, testgestuetzt fuer `MEDISTAR + NIDEK AR360` und als LM7-Kandidat fuer `MEDISTAR + NIDEK LM7`.
...
| NIDEK | LM7 / LM-7P | Lensmeter | praktisch nutzbarer Kandidat, MEDISTAR-Praxisvalidierung offen | `XmlDeviceParser`, NIDEK-LAN-XML `NIDEK_V1.00/V1.01`, akzeptiert `Sphere/Sphare` und `NearSphere/NearSphare` | BuiltIn `device-nidek-lm7-default` plus BuiltIn-Schnittstellenprofil | echte LM7-Fixture, Stylesheet-Ignoriertest, MEDISTAR-Lensmeter-Ausgabe, BuiltIn- und selektive LM7-TemplatePackage-Tests | Kandidat dokumentiert; selektiver Export/Import reproduzierbar testgeprueft; ZIP-Release-Artefakt noch offen | MEDISTAR | nein / nicht validiert | MEDISTAR-Live-Test mit echter LM7-Datei und danach ggf. Release-Regel anwenden | Prisma-/PD-Faelle bleiben zu validieren; keine Werte aus Formatbeispielen uebernehmen |
...
| MEDISTAR + NIDEK LM7 | MEDISTAR | `device-nidek-lm7-default` | `export-medistar-nidek-lm7-default` | `interface-medistar-nidek-lm7-default` | ja, `docs/TEMPLATEPAKET_MEDISTAR_NIDEK_LM7.md` | ja, `MedistarNidekLm7TemplatePackageTests` | nein, steht noch aus | nein | nein | MEDISTAR-Live-Test mit echter LM7-Datei; Prisma/PD-Faelle sammeln |
...
3. NIDEK LM7/LM7P praktisch in MEDISTAR live pruefen: die echte XML-Fixture ist testseitig abgesichert, das Templatepaket ist vorbereitet, Praxisabnahme und offizielles ZIP fehlen noch.
...
- Fuer LM7/LM7P fehlt noch die praktische MEDISTAR-Abnahme; fuer NT530P und TOPCON fehlen Repository-Testdaten oder praktische Abnahmeprotokolle.
