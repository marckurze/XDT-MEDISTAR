...
| MEDISTAR + NIDEK NT530P | MEDISTAR | `device-nidek-nt530p-default` | `export-medistar-nidek-nt530p-default` | `interface-medistar-nidek-nt530p-default` | ja, `docs/TEMPLATEPAKET_MEDISTAR_NIDEK_NT530P.md` | ja, `MedistarNidekNt530PTemplatePackageTests` | nein, noch offen | nein, JPG-Anhangfall offen | nein | Praktischen MEDISTAR-Importlauf fuer `6220`/`6205` abnehmen |

## Weitere vorbereitete Geraete

| Hersteller | Geraet | Typ | Status | Parser/Reader | Geraeteprofil | Tests | Templatepaket | AIS-Ziel | XDT-Anhang | Naechste sinnvolle Massnahme | Risiko / Unsicherheit |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| NIDEK | NT530P | Tonometer / Pachymeter | direkt nutzbarer Kandidat, praktische MEDISTAR-Validierung offen | `XmlDeviceParser`, NIDEK-XML Root `Data`, `NT`, `CorrectedIOP`, `PACHY`, berechnete MEDISTAR-Linien | BuiltIn `device-nidek-nt530p-default` plus BuiltIn-Schnittstellenprofil | echte NT530P-Fixture, JPG-nicht-DeviceXml-Test, `6220`/`6205`-Export, Reparatur alter BuiltIn-NT530P-Exportpfade, selektive NT530P-TemplatePackage-Tests | Kandidat dokumentiert; selektiver Export/Import reproduzierbar testgeprueft; ZIP-Release-Artefakt noch offen | MEDISTAR | ja, wegen JPG-/Bildverweisen relevant, aber nicht produktiv validiert | Praktischen MEDISTAR-Importlauf fuer Pachymetrie/Tonometrie und optionalen JPG-Anhangfall abnehmen | Attachment-Zuordnung und Karteikarten-Darstellung fuer `6205`/`6220` muessen praktisch bestaetigt werden |
...
4. Danach NIDEK NT530P praktisch in MEDISTAR abnehmen: Tonometrie/Pachymetrie laufen ueber `6205`/`6220`, JPG-Aufnahmen bleiben als separater Anhangfall offen. TOPCON CL300/KR800/TRK2P folgen nur mit belastbaren Beispiel- und Testdaten.
...
- Fuer LM7/LM7P ist die Lensmeter-XDT-Rueckgabe praktisch validiert; offen bleiben weitere Prisma-/PD-Beispielfaelle, ein separat validierter XDT-Anhang-Link und ein offizielles ZIP-Release-Artefakt. Fuer NT530P ist die echte XML-Fixture testseitig abgesichert; offen bleibt die praktische MEDISTAR-Abnahme. Fuer TOPCON fehlen Repository-Testdaten oder praktische Abnahmeprotokolle.
- Bei TOPCON ist Namespace-/JOIA-Verhalten noch gezielt mit echten Dateien zu pruefen.
- Bei NT530P sind die Feldkennungen `6220` und `6205` testseitig gesetzt; Tonometrie/Pachymetrie-Anzeige und optionale JPG-Anhaenge sind praktisch zu validieren. Bei TRK2P bleiben Tonometrie/Pachymetrie-Ausgabe und Einheiten fachlich zu pruefen.
