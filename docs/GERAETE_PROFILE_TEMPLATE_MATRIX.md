...
- BuiltIn-Geraeteprofile existieren fuer NIDEK ARK1S, NIDEK AR360 / AR-360A, NIDEK LM7, NIDEK NT530P, TOPCON CL300, TOPCON KR800, TOPCON TRK2P und das generische Dokumentgeraet.
- BuiltIn-MEDISTAR-Exportprofile existieren fuer dieselben acht Geraete beziehungsweise Workflows.
- Fertige BuiltIn-Schnittstellenprofile existieren aktuell fuer `MEDISTAR + NIDEK ARK1S`, `MEDISTAR + NIDEK AR360`, `MEDISTAR + NIDEK LM7`, `MEDISTAR + NIDEK NT530P` und `MEDISTAR + Dokumentanhang`.
- Die Templatepaket-Infrastruktur ist vorhanden und testseitig abgesichert. Fuer MEDISTAR + NIDEK ARK1S, MEDISTAR + NIDEK AR360, MEDISTAR + NIDEK LM7, MEDISTAR + NIDEK NT530P und MEDISTAR + Dokumentanhang gibt es Referenz- beziehungsweise Kandidatendokumentation; der Export erfolgt selektiv aus dem Schnittstellenprofil und nimmt nur die benoetigten Abhaengigkeiten auf. ZIP-Struktur, Import/DryRun und sicherer UserDefined-Import sind reproduzierbar im Test geprueft. Dauerhafte ZIP-Release-Artefakte werden erst nach `docs/TEMPLATEPAKET_RELEASE_REGEL.md` abgelegt.
...
- Repository-Testdaten enthalten `nidek-ark1s-sample.xml`, AR360-Beispielfixtures, die echte LM7-XML-Fixture `NIDEK LM7.xml`, das zu ignorierende `NIDEK_LM_Stylesheet.xsl`, die echte NT530P-XML-Fixture `NIDEK_NT530P.xml` sowie generische GDT-Beispiele; fuer TOPCON liegen im Repository noch keine vollstaendigen Geraete-Beispieldateien.
...
| Generisch | Dokumentgeraet | Dokument-/Dateianhang ohne Messwerte | V1-Kandidat, testseitig direkt nutzbar | kein Messwertparser; AttachmentOnly verarbeitet stabile Dateien aus dem Geraete-/Dokument-Importordner als Anhaenge und erzwingt die Linkvorbereitung auch bei alten deaktivierten technischen Anhang-Flags | BuiltIn `device-document-attachment-default` plus BuiltIn-Schnittstellenprofil | ImportFileClassifier-, AttachmentScanner-, Completion-, ManualProcessor-, AutoImportScanner- und TemplatePackage-Tests | Kandidat dokumentiert; selektiver Export/Import reproduzierbar testgeprueft; ZIP-Release-Artefakt noch offen | MEDISTAR | ja, mehrere Anhaenge je Vorgang ueber `6302`-`6305`; optionaler mehrzeiliger `6227` Dokumentationstext | Praktischen MEDISTAR-Live-Test mit PDF/JPG/XML/Medienanhaengen, Textoption an/aus, Ruhezeit-/Bestaetigungsmodus und optionalem Text durchfuehren | Keine Vorschau/Drag-&-Drop/OCR; XML wird in diesem Modus nicht als Messwert geparst |
...
6. TOPCON CL300/KR800/TRK2P folgen nur mit belastbaren Beispiel- und Testdaten.

## Luecken fuer direkte Nutzung ohne Baukasten

- Fertige Templatepaket-Dateien fehlen noch; fuer ARK1S, AR360, LM7 und Dokumentanhang sind Vorlage/Kandidat, selektiver Export und technischer Export-/Import-Testweg vorhanden.
...
- Fuer LM7/LM7P ist die Lensmeter-XDT-Rueckgabe praktisch validiert; offen bleiben weitere Prisma-/PD-Beispielfaelle, ein separat validierter XDT-Anhang-Link und ein offizielles ZIP-Release-Artefakt. Fuer NT530P ist die echte XML-Fixture mit korrigiertem `6205`-/`6220`-Layout testseitig abgesichert; offen bleibt die praktische MEDISTAR-Abnahme. Fuer TOPCON fehlen Repository-Testdaten oder praktische Abnahmeprotokolle.
- Bei TOPCON ist Namespace-/JOIA-Verhalten noch gezielt mit echten Dateien zu pruefen.
- Bei NT530P sind die Feldkennungen `6220` und `6205` mit Headern und Einzelzeilen testseitig gesetzt; Tonometrie/Pachymetrie-Anzeige und optionale JPG-Mehrfachanhaenge sind praktisch zu validieren. Bei TRK2P bleiben Tonometrie/Pachymetrie-Ausgabe und Einheiten fachlich zu pruefen.
