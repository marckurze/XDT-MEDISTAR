...
Fuer rein manuelle Arbeitsablaeufe gibt es zusaetzlich `MEDISTAR + Manuelle Dokumentuebergabe`. Erst eine AIS-Datei startet dabei den Dialog `Dokumente uebertragen` ohne erwartete Geraetedatei; das gruene Geraeteanbindungsfenster bleibt fuer diesen Sondermodus angedockt. Der Anwender fuegt Dateien per Drag-&-Drop, Dateiauswahl oder Datei-Zwischenablage hinzu. Manuell ausgewaehlte Quelldateien werden nicht geloescht.
