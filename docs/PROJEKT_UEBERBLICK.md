...
Die Installations-/Update-Datenpolitik fuer einen spaeteren Installer steht in `docs/INSTALLATION_UPDATE_DATENPOLITIK.md`. App-Dateien, Hilfe, Themes, Standard-Assets und BuiltIn-Definitionen sind ersetzbare App-Bestandteile. Kundendaten unter `%LocalAppData%\XdtDeviceBridge` bleiben bei Updates erhalten: Profile, UserDefined-Praxisparameter, Baukasten-Templates, lokale Geraetebilder, `device-image-overrides.json`, Lizenzdaten inklusive `license.xdtboxlic`, UI-Komfortdaten und Backups. Deinstallation folgt Variante B: App entfernen, Kundendaten behalten, Loeschen nur nach ausdruecklicher separater Bestaetigung; externe Praxisordner werden nie blind geloescht. Das interne Hersteller-Lizenztool und private Schluessel gehoeren nicht in den Kundeninstaller.
