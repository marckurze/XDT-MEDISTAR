...
Exportregeln im Baukasten sind nur Arbeitskopien. Plus/Muelltonne aendern nicht das BuiltIn-Original, feste Ueberschriften oder Notizen duerfen einen leeren `SourcePath` haben, solange ein Regeltext vorhanden ist. Platzhalter zeigen rechts den aktuell eingelesenen Beispielwert und aktualisieren die Vorschau nach dem Einfuegen; Geraeteplatzhalter werden nur aus zum aktuellen Geraeteprofil passenden Testdaten befuellt, damit Werte nicht unter falschen Profilen erscheinen. Die zentrale Baukasten-Kompatibilitaet prueft bekannte BuiltIn-Modelle alias-tolerant, laesst UserDefined-/Entwurfsprofile aber offen, damit neue Geraete anhand geparster Messpfade aufgebaut werden koennen. Der Undo-Pfeil nimmt Baukasten-Aenderungen schrittweise zurueck. `Templatepaket laden` ist bis zur lokalen Paketbibliothek sichtbar als vorbereitete V1-Funktion gekennzeichnet.
