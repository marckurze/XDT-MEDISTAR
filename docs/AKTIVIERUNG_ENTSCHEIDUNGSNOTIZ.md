...
- `InterfaceProfileActivationExecutorResult` kann fehlende produktive Faehigkeiten nun ausdruecken: frisches Laden erforderlich, sichere UserDefined-Speicherung erforderlich, finale Re-Evaluation erforderlich, einzelne ValidateOnly-Schritte ausgefuehrt/nicht ausgefuehrt, Save-DryRun ausgefuehrt/blockiert, nicht ausgefuehrt, nicht gespeichert, kein Profil geaendert und fehlende Capabilities.
...
- Optional kann der Stub im `ValidateOnly`-Modus einen `IInterfaceProfileActivationProfileStore` verwenden: Dann wird das Zielprofil frisch geladen, `NotFound`, BuiltIn und Nicht-UserDefined werden ueber den Store bewertet.
- Optional kann der Stub danach die finale Pruefkette simulieren, wenn `InterfaceProfileActivationEvaluationService`, `InterfaceProfileActivationGuardService`, `InterfaceProfileActivationWarningConfirmationService`, `InterfaceProfileActivationPlanService` und ein aktueller `ProfileCatalog` injiziert wurden. Fehlt eine dieser Abhaengigkeiten, meldet der Stub eine MissingCapability und der Save-DryRun bleibt blockiert.
- Save-DryRun wird nur nach erfolgreicher finaler ValidateOnly-Pruefung freigegeben und bleibt weiterhin nicht-produktiv.
