...
Der Tab `Profile & Templates` enthält einen schrittweise organisierten Bereich `Test & Vorschau` für den Baukasten-Test: AIS-Testdatei laden, Gerätedatei laden, optional XDT-Anhang-Diagnose ausführen, Messwerte prüfen, Gesamtexport-Vorschau kontrollieren und eine Test-XDT-Datei schreiben. Der Tab `Verarbeitung` bleibt der Betriebsbereich für aktive Schnittstellenprofile, manuell gestartete Überwachung und automatische Verarbeitung. Der bisherige manuelle Testbereich ist dort weiterhin als eingeklappter Diagnose-/Rückfallbereich `Diagnose / manueller Alt-Test` erhalten.
