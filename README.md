...
Die Oberflaeche ist inzwischen staerker auf Techniker- und Supportarbeit ausgerichtet: Schnittstellenprofile zeigen direkt Geraetebild, AIS-Ausgabeinfo, Geraetesteckbrief und einen Aktivierungsstatus; der XDT-Baukasten kann denselben Geraetesteckbrief aus der Geraeteidentitaet oeffnen. Zusaetzlich gibt es den Kundendaten-Tab `Notizen Techniker` fuer freie Whiteboard-Notizen, Bilder, aktive Geraeteuebersichten und PDF-Export sowie einen optionalen lokalen Tab-Schutz fuer alle Tabs ausser `Verarbeitung`. Das moderne Windows-Erscheinungsbild wird zentral ueber `XdtBoxTheme.xaml` gesteuert: XDTBox-Farben, Kartenflaechen, Tabs, Buttons, Eingabefelder, Tabellen und Toggle-Schalter teilen sich eine gemeinsame, ruhige Praxissoftware-Optik.
...
Der Tab `Notizen Techniker` ist ein lokaler Kundendatenbereich. Er speichert ein einfaches Whiteboard mit Texten, Bildern, Positionen, Groessen und Formatierungen unter `%LocalAppData%\XdtDeviceBridge\technician-notes`. Bilder werden in den Kundendatenordner kopiert, koennen per Drag-and-Drop eingefuegt und mit der Maus positioniert beziehungsweise skaliert werden. Aktive Geraeteanbindungen koennen als Textblock eingefuegt werden; der Whiteboard-Inhalt kann als PDF exportiert werden.
...
- Techniker-Whiteboard fuer installationsbezogene Notizen, Bilder, aktive Geraeteuebersicht und PDF-Export.
