...
- Serielle bidirektionale Phoropter-Kandidaten NIDEK RT-2100 / RT-3100 / RT-5100: XDTBox kennt PDF-basierte RS232-Presets, kann synthetische dokumentnahe RT->PC-Rohdaten sowie einen ersten echten RT-3100-Praxismitschnitt parsen, `Final -> 6228` und `Subjective -> 6227` abbilden und den produktiven Ablauf patientengetriggert fuehren: AIS-Datei kommt an, Auswahlfenster oeffnet, der Anwender sendet LM-/AR-Historienwerte bewusst ueber COM-Port, danach wartet XDTBox bis `EOT` plus Stabilitaetswartezeit auf die RT-Rueckgabe. Der echte RT-3100-Mitschnitt bestaetigt `SH Header CR`, zeilenweise `STX @RT`/Datenzeilen, `EOT CR`, Final-R/L-Werte, ADD, VA, PD und WorkingDistance. Fuer die Praxisabnahme zeigt das RT-Fenster eine einklappbare serielle Diagnose mit COM-Parametern, DTR/RTS/Handshake, RS-/SD-/Writer-Hexdump, sichtbaren Steuerzeichen und Timeout-Checkliste; `COM-Port nur abhoeren` prueft die Profilparameter ohne Sendung und ohne Export. Echtes Senden, Rueckgabe nach Sendung und MEDISTAR-Import bleiben praktisch am Geraet zu pruefen.
