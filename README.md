...
Die Oberflaeche ist inzwischen staerker auf Techniker- und Supportarbeit ausgerichtet: Schnittstellenprofile zeigen direkt Geraetebild, AIS-Ausgabeinfo, Geraetesteckbrief und einen grossen Aktivierungsstatusbutton; der XDT-Baukasten kann denselben Geraetesteckbrief aus der Geraeteidentitaet oeffnen. Zusaetzlich gibt es den Kundendaten-Tab `Notizen Techniker` fuer freie Whiteboard-Notizen, Bilder, aktive Geraeteuebersichten und PDF-Export sowie einen optionalen lokalen Tab-Schutz fuer alle Tabs ausser `Verarbeitung`.
