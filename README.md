...
Neu ist der zusätzliche Tab `XDT-Baukasten`. Er ist als eigenständige Arbeitsoberfläche für Entwürfe und Testdaten aufgebaut und soll den bisherigen Sammelbereich später ersetzen. Der neue Baukasten hält AIS-, Geräte- und Exportprofil, Rohdaten, optionalen Testanhang, Exportregel-Arbeitskopie und Vorschauzustand getrennt von produktiven Schnittstellenprofilen. `Verarbeitung starten` erzeugt dort ausschließlich eine Vorschau im Speicher; es wird keine produktive Datei geschrieben, nichts archiviert und keine Überwachung gestartet. Die Ergebnisbox bietet `Roh-XDT`, `Ansicht im AIS`, `Geräteausgabe` und `Diagnose`; `Ansicht im AIS` zeigt nur fachlich sichtbare Karteikartenzeilen ohne Patientendatenblock, Feldnummern oder Parserdiagnose. Exportregeln können in der Arbeitskopie ergänzt/entfernt werden, feste Notizen funktionieren ohne `SourcePath`, Platzhalter zeigen rechts aktuelle Beispielwerte aus der passend geladenen Testdatei und der Undo-Pfeil nimmt Baukasten-Änderungen zurück. Die Gerätekompatibilität nutzt eine zentrale Prüfung: bekannte BuiltIn-Profile schützen gegen falsche Geräte-Dateien und akzeptieren Modell-Aliase wie CV-5000/CV-5000S, während UserDefined-/Entwurfsprofile für neue Geräte geparste Messpfade permissiv als Baukasten-Platzhalter anzeigen. `Templatepaket laden` ist in V1 sichtbar als vorbereitete lokale Paketbibliothek gekennzeichnet.
