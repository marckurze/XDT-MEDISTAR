...
Die Oberflaeche ist inzwischen staerker auf Techniker- und Supportarbeit ausgerichtet: Schnittstellenprofile zeigen direkt Geraetebild, AIS-Ausgabeinfo, Geraetesteckbrief und einen Aktivierungsstatus; die unteren Bereiche fuer Ordnerbereinigung, Archivierung und Aktivierungspruefung sind layoutseitig getrennt und verhindern Ueberlagerungen zwischen Status, Buttons, Hinweisen und Prueftabellen. Der XDT-Baukasten kann denselben Geraetesteckbrief aus der Geraeteidentitaet oeffnen. Zusaetzlich gibt es den Kundendaten-Tab `Notizen Techniker` fuer freie Whiteboard-Notizen, Bilder, aktive Geraeteuebersichten und PDF-Export; Text-/Handmodus sind als Symbolschalter ausgelegt, Texte koennen direkt geschrieben und spaeter per Doppelklick weiterbearbeitet werden. Der Lizenzbereich trennt Praxisdaten, Kontakt, Rechnungsversand sowie Bank/SEPA sauber, wobei SEPA-Zustimmung und Rechnung-ohne-SEPA direkt nebeneinander stehen. Das moderne Windows-Erscheinungsbild wird zentral ueber `XdtBoxTheme.xaml` gesteuert: XDTBox-Farben, Kartenflaechen, Tabs, Buttons, Eingabefelder, Tabellen und Toggle-Schalter teilen sich eine gemeinsame, ruhige Praxissoftware-Optik.
...
Lokale Anpassungen werden nicht ins Original geschrieben. `DeviceTechnicalProfileService` fuehrt Original und lokale Overrides aus `%LocalAppData%\XdtDeviceBridge\device-info-overrides.json` zusammen. Die Steckbriefansicht kann Kurzbeschreibung, technische Hinweise und die `Notiz Techniker zum Geraet` lokal speichern und wieder auf das Original zuruecksetzen. Sie ist im Tab `Schnittstellenprofile` und im `XDT-Baukasten` ueber `Geraetesteckbrief` erreichbar. Sicherung/Umzug nimmt diese lokale Override-Datei mit; eine Kopplung an den Profil-/Templateexport ist vorbereitet und als Folgeschritt offen. Die gesammelte Pruefansicht fuer Marc steht in [`docs/GERAETE_STECKBRIEFE.md`](docs/GERAETE_STECKBRIEFE.md).

## Techniker-Notizen und Tab-Schutz

Der Tab `Notizen Techniker` ist ein lokaler Kundendatenbereich. Er speichert ein einfaches Whiteboard mit Texten, Bildern, Positionen, Groessen und Formatierungen unter `%LocalAppData%\XdtDeviceBridge\technician-notes`. Der Whiteboard-Bereich unterscheidet Textmodus und Handmodus: Im Textmodus legt ein Klick auf die Flaeche direkt einen editierbaren Text an, im Handmodus werden Texte und Bilder ausgewaehlt, verschoben und ueber sichtbare Griffe skaliert. Das Kontextmenue bietet Kopieren, Einfuegen und Entfernen; eingefuegte Bilder werden weiterhin in den Kundendatenordner kopiert. Aktive Geraeteanbindungen koennen als Textblock eingefuegt werden; der Whiteboard-Inhalt kann als PDF exportiert werden.

Der optionale Tab-Schutz sperrt alle Haupttabs ausser `Verarbeitung`. Er ist standardmaessig aus und wird ueber das Zahnrad konfiguriert. Das lokale Passwort wird nicht im Klartext gespeichert, sondern mit Salt und Hash unter `%LocalAppData%\XdtDeviceBridge\ui\tab-protection.json`. Sicherung/Umzug nimmt diese Konfiguration mit.

## Installation, Update und Kundendaten

XDTBox 1.12 besitzt ein reproduzierbares Inno-Setup-Skript und eine Buildanleitung in [`docs/INSTALLER_BUILD_ANLEITUNG.md`](docs/INSTALLER_BUILD_ANLEITUNG.md). Der Default-Installationspfad ist `C:\XDTBox`, der Benutzer kann ihn bei Neuinstallation aendern. Das Setup unterscheidet Neuinstallation und Update, erzeugt `XDTBox_Setup_1.12.exe`, nutzt das offizielle `XDTBox.ico`, legt Startmenue- und optional Desktop-Verknuepfungen an und traegt XDTBox in Windows "Programme & Features" ein.
...
- Geraetesteckbriefe direkt aus `Schnittstellenprofile` und `XDT-Baukasten`, mit App-Originaldaten plus lokalen Overrides.
- Techniker-Whiteboard fuer installationsbezogene Notizen, Bilder, aktive Geraeteuebersicht, Text-/Handmodus, Kontextmenue und PDF-Export.
- Optionaler lokaler Tab-Schutz fuer alle Tabs ausser `Verarbeitung`.
- Erweiterte Lizenz-Kundendaten fuer Rechnung, SEPA und Bankdaten, ohne Aenderung der Lizenzsignaturlogik.
...
- Serielle bidirektionale Phoropter NIDEK RT-2100 / RT-3100 / RT-5100: XDTBox kennt PDF-basierte RS232-Presets, kann synthetische dokumentnahe RT->PC-Rohdaten sowie echte RT-3100-Praxismitschnitte parsen, `Final -> 6228` und `Subjective -> 6227` abbilden und den produktiven Ablauf patientengetriggert fuehren. Fuer den RT-3100-Praxisablauf ist der komplette Lauf praktisch bestaetigt: `An RT-3100 senden` nutzt `RS senden, dann Writer ohne SD`, die spaetere Rueckgabe wird empfangen, mit dem AIS-Kontext gemappt und als MEDISTAR-XDT ausgegeben. Nach terminalem Erfolg wird der RT-Workflow geschlossen/gedockt und nicht durch nachlaufende Success- oder Refresh-Ereignisse erneut geoeffnet; erst eine neue AIS-Patientendatei startet wieder mit dem Auswahlfenster. RT-2100 und RT-5100 verwenden denselben vorbereiteten produktiven Sendemodus, sind aber noch als Einzelgeraete live abzunehmen. Zusaetzlich ist der `NIDEK-RT Sendeinhalt` speicherbar und steht standardmaessig auf `RT-Referenz ohne ID (AR/AL)`, einer aus produktiven RT-Anbindungen abgeleiteten Frameform ohne ID-Block, mit LM ADD `AR`/`AL` und Abschluss `EB ET`. Die `RT-3100 Praxisvariante (getestet)` bleibt als Legacy-Testform mit 107-Byte-Frame, Legacy-ADD `RA`/`LA` und ohne zusaetzliches `EB` vor `ET` erhalten. Die RS-Anforderung sendet echte ASCII-Sternchen (`01 43 2A 2A 02 52 53 17 04`); LM-SCA-Augenpraefixe bleiben dagegen Leerzeichen + `R`/`L`. Fuer die Praxisabnahme zeigt das RT-Fenster eine einklappbare serielle Diagnose mit COM-Parametern, DTR/RTS/Handshake, CTS/DSR/DCD/RI, RS-/Writer-Hexdump, sichtbaren Steuerzeichen und Timeout-Checkliste. `COM-Port nur abhoeren` prueft die Profilparameter ohne Sendung und ohne Export; `Rueckgabe abhoeren und verarbeiten` nutzt dagegen den wartenden Patientenkontext und erzeugt nach gueltiger Rueckgabe produktiv die MEDISTAR-XDT-Datei.
...
- Die grafische Hersteller-App `XdtBox.LicenseManager.exe` heisst sichtbar `XDTBox Lizenzmanager`, besitzt eine eigene Version `1.11` und liest Lizenzanfragen, erzeugt signierte `.xdtboxlic`-Dateien, fuehrt eine lokale Historie ausgestellter Lizenzen und speichert Hersteller-Einstellungen. Sie ist nicht Teil der Endkunden-App; private Schluessel bleiben externe Dateien.
- Aktuell erzeugte Lizenzen sind unbefristet gueltig, solange InstallationId/Hardware passt und die Signatur gueltig ist. Anzeigen wie `Gueltig bis` schreiben dafuer `unbefristet`; es gibt keine Abo-, Monats- oder Onlinepruefung.
- `XdtBox.LicenseManager.exe` fuehrt zusaetzlich eine lokale Kundenliste aus Lizenzanfragen. Kunden werden ueber Kundennummer oder InstallationId angelegt beziehungsweise aktualisiert, koennen mehrere Installationen enthalten und zeigen nur aktive lizenzpflichtige Geraeteanbindungen aus der Anfrage/Historie.
- Rechnungs-E-Mail ist von der Zahlungsart getrennt. Die Zahlungsart ist entweder SEPA-Lastschrift oder Bankueberweisung; IBAN/Kontoinhaber werden nur fuer SEPA plausibilisiert.
- Ein einstellbarer Netto-Einzelpreis pro Geraeteanbindung berechnet Kunden- und Gesamtkosten. Die Kundenliste kann als grafische A4-Querformat-PDF-Tabelle exportiert werden; Kundendetails zeigen Stammdaten, getrennte Installationen, aktuelle lizenzierte Anbindungen und Lizenzhistorie.
- Im Tab `Kunden` koennen zwei oder mehr Kunden per Checkbox markiert und ueber den kurzen Button `Zusammenführen` zu einem Zielkunden zusammengefuehrt werden. Der Dialog waehlt den Hauptkunden sowie die zu uebernehmenden Stammdaten; Installationen, aktive lizenzierte Anbindungen inklusive Geraetestandort und die Lizenzhistorie bleiben ueber InstallationId erhalten. Doppelte InstallationIds werden zusammengefuehrt statt doppelt angelegt.
- Die Kundenliste zeigt zusaetzlich die Gesamtsumme monatlicher Lizenzen aus allen aktiven lizenzpflichtigen Geraeteanbindungen mal Einzelpreis. `Ausgestellte Lizenzen` trennt reduzierte Historienliste und strukturierte Details; lokale Historieneintraege koennen entfernt werden, ohne Lizenzdateien, private Schluessel oder Kundenstammdaten zu loeschen.
- Beim Erstellen einer Lizenz werden nur aktive lizenzpflichtige Geraeteanbindungen aus der Lizenzanfrage angezeigt und uebernommen. Alte Lizenzanfragen ohne Geraeteliste bleiben importierbar und fallen auf die angefragte Anzahl aktiver Lizenzplaetze zurueck.
- Eine Lizenz/Installation kann im XDTBox Lizenzmanager lokal als storniert markiert werden. Das reduziert aktive Geraete und Kosten in der Herstellerverwaltung, macht aber eine bereits beim Kunden installierte Offline-Lizenzdatei nicht automatisch ungueltig.
- Die LicenseManager-Sicherung enthaelt Kundenliste, Bank-/SEPA-/Rechnungsdaten, Preis, Einstellungen, Storno-Status und Lizenzhistorie. Private Keys, Signaturzertifikate und Hersteller-Master-Keys werden bewusst nicht gesichert.
- Ein eigenes Setup `XDTBox_Lizenzmanager_Setup_1.11.exe` wird ueber `scripts\build-xdtbox-licensemanager-installer.ps1` gebaut und installiert nur den XDTBox Lizenzmanager nach `C:\XDTBox\Lizenzaktivierung`; Kunden-App, private Schluessel, vorhandene Kundendaten, Backups und History-Dateien werden nicht mitgeliefert.
...
- Ergebnisformate sind fuer die dokumentierten Praxisreferenzen wie NIDEK ARK1S, AR360, LM7/LM7P, NT530P und RT-3100 RS232 praktisch validiert; weitere BuiltIns bleiben bis zur eigenen Praxisabnahme Kandidaten.
