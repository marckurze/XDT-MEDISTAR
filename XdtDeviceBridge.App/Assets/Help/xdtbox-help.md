...
XDTBox 1.12 wird als klassisches Windows-Setup installiert. Der vorgeschlagene Installationsordner ist C:\XDTBox und kann bei Neuinstallation geändert werden.
...
Regeln können über das Plus ergänzt und über die Mülltonne aus der Arbeitskopie entfernt werden. Ein leerer SourcePath ist für feste Überschriften oder Notizen erlaubt, wenn im Regeltext ein fester Text steht. Platzhalter zeigen rechts den aktuell eingelesenen Beispielwert; Geräteplatzhalter stammen aus der aktuell geladenen Testdatei. XDTBox übernimmt vom Gerät gelieferte Einzelwerte und stellt sie als Platzhalter bereit, ohne Messwerte zu berechnen. Manuelle Änderungen im Regeltext werden live in die Arbeitskopie übernommen und aktualisieren die Vorschau sofort. Bei bidirektionalen Geräten zeigen Ausgabe-an-Gerät-Platzhalter Patientendaten und verfügbare historische Werte für die Geräteausgabe. Änderungen an unterstützten Geräteausgabe-Regeln aktualisieren die Geräteausgabe-Vorschau sofort. Im Baukasten blockieren Modellabweichungen die Vorschau nicht, solange die Datei lesbar ist und Parser/Mapping verwertbare Daten liefern. XDTBox zeigt dann eine Warnung und schreibt die Abweichung in die Diagnose, damit neue Geräte, ähnliche Modelle und Exportprofile bewusst verglichen werden können. Die produktive Verarbeitung prüft strenger. Ein Klick fügt den Platzhalter in den Entwurf ein und aktualisiert die Vorschau. Der Zurück-Pfeil nimmt die letzten Baukasten-Änderungen schrittweise zurück. BuiltIn-Profile werden dadurch nicht direkt überschrieben.
...
Canon RK-F2, Canon TX-20P, ZEISS VISULENS 550, ZEISS VISUPLAN 500, ZEISS VISUREF 100, ZEISS IOLMaster 700, Visionix Retinomax 5, Visionix VX 120 und Visionix VX 650 sind als inaktive BuiltIn-Kandidaten vorbereitet. Die Ausgaben folgen den XDTBox-Regeln: REF/Lensmeter nach `6228`, KM nach `6221`, Tonometrie nach `6205`, Pachymetrie/CCT nach `6220`, IOLMaster-VKT/AL nach `6227` und IOLMaster-R1/R2 nach `6228`.
...
NIDEK NT530P / NT-530P ist fuer die Tonometrie-/Pachymetrie-XDT-Rueckgabe praktisch validiert. Fehlerhafte oder unvollstaendige Daten erzeugen keine kuenstlichen Messwerte. JPG- und Mehrfachanhangfaelle bleiben separate Praxisabnahmen.
...
TOPCON KR800S ist als praktisch validiertes Mehruntersuchungsgerät vorbereitet. Autorefraktion, Keratometrie, SBJ und VA werden nur aus vorhandenen Gerätedaten übernommen. XDTBox berechnet keinen Visus; wenn das Gerät VA liefert, steht VA als Einzelplatzhalter im Baukasten bereit und wird zusätzlich in der REF-`6228`-Zeile ausgegeben.
...
Die RT-Presets setzen DTR und RTS standardmaessig aktiv. Beim RT-3100 wurde im Praxisaufbau bestaetigt, dass mit DTR aus keine Rueckgabe empfangen wurde, mit DTR aktiv aber ein vollstaendiger Frame ankam. Der bestaetigte Sendepfad fuer den RT-3100 sendet zuerst RS, wartet nur kurz und sendet den Writer-Frame danach ohne harte SD-Pflicht.

Im Tab "Schnittstellenprofile" gibt es fuer diese seriellen RT-Profile die Auswahl "NIDEK-RT Sendemodus":

- "RS senden, dann Writer ohne SD": XDTBox sendet RS, wartet kurz und sendet den Writer-Frame auch ohne SD. Das ist der aktuelle Praxisdefault fuer den RT-3100-Produktivablauf.
- "RS/SD-Handshake": XDTBox sendet zuerst RS, erwartet SD und sendet den Writer-Frame nur bei Bestaetigung.
- "Direkt Writer-Frame senden": XDTBox sendet den Writer-Frame direkt, erwartet keine SD-Bestaetigung und wartet danach auf die spaetere Rueckgabe.
...
Im produktiven Ablauf oeffnet XDTBox das RT-Fenster nicht schon beim Start der Ueberwachung. Erst wenn eine AIS-Patientendatei ankommt, werden Patient und Historie gelesen und das Auswahlfenster im Vordergrund geoeffnet. Dort koennen Lensmeter- und Autorefraktor-Historienwerte ausgewaehlt werden. Senden erfolgt nur nach Klick auf "An RT senden" ueber den im Schnittstellenprofil gepflegten COM-Port; beim RT-3100 laeuft dann automatisch der Pfad RS plus Writer ohne SD-Pflicht. Nach "Senden" oder "Nichts senden" wird die eine vorhandene RT-Geraetekachel fokussiert und wartet mit Patientenkontext, bis die Phoropter-Rueckgabe empfangen und bis EOT plus kurzer Stabilitaetswartezeit verarbeitet wurde. Nach erfolgreicher Rueckgabe schliesst sich die Kachel automatisch und bleibt fuer diesen abgeschlossenen Zyklus geschlossen. Erst eine neue AIS-Patientendatei startet wieder mit dem Auswahlfenster.

Das RT-Fenster enthaelt fuer die Praxisabnahme eine einklappbare serielle Diagnose. Sie zeigt COM-Port, Baudrate, Datenbits, Paritaet, Stoppbits, Flusskontrolle, DTR/RTS, CTS/DSR/DCD/RI, RS-Anforderung, den PC->RT-Writer-Frame, Hexdump, sichtbare Steuerzeichen und den Empfang bis EOT. Die RS-Anforderung wird als `<SOH>C**<STX>RS<ETB><EOT>` beziehungsweise `01 43 2A 2A 02 52 53 17 04` gesendet. LM-SCA-Augenpraefixe werden dagegen als Leerzeichen + `R`/`L` gesendet, nicht als ASCII-Sternchen. Die RT-Referenz ohne ID (AR/AL) sendet ohne ID-Block, LM ADD als `AR`/`AL` und endet mit `EB ET`; die RT-3100 Praxisvariante reproduziert dagegen bewusst den live angenommenen 107-Byte-Frame mit Legacy-ADD `RA`/`LA` und ohne zusaetzliches `EB` direkt vor `EOT`. "COM-Port nur abhoeren" oeffnet denselben Profil-Port, sendet nichts und erzeugt keinen XDT-Export. "Rueckgabe abhoeren und verarbeiten" ist der produktive Rueckweg im Wartestatus: Die Funktion sendet nichts, nutzt aber den gespeicherten AIS-Kontext und erzeugt bei gueltiger Rueckgabe die MEDISTAR-XDT-Datei.

Die frueheren RT-Sendetestbuttons sind in der normalen Praxis-Geraetekachel ausgeblendet. Der zuvor erfolgreiche Pfad "RS + Writer ohne SD-Warten" wird fuer den RT-3100 jetzt automatisch durch den produktiven Button "An RT-3100 senden" verwendet. Wenn der RT einen kombinierten Vollframe nicht uebernimmt, bleiben die gespeicherten Sendeinhaltsvarianten im Schnittstellenprofil fuer die Praxisabnahme verfuegbar: zuerst RT-Referenz ohne ID (AR/AL), danach RT-3100 Praxisvariante, Nur Lensmeter ohne ADD, Nur Lensmeter, Nur Autoref und Varianten ohne ID.

Echte Daten vom RT werden aus RS232-Rohdaten geparst; Final-Werte werden fuer MEDISTAR als 6228 und Subjective-Werte als 6227 vorbereitet. Ein Geraete-Eingangsordner und ein dateibasierter Ausgabeordner an das Geraet sind fuer diese seriellen Phoropter nicht erforderlich. Wenn nach dem Senden nicht sofort eine Rueckgabe kommt, bleibt XDTBox im Wartestatus: Fuehren Sie die Untersuchung am Phoropter durch, loesen Sie danach PRINT/SEND aus und starten Sie bei Bedarf "Rueckgabe abhoeren und verarbeiten". Ohne Rueckgabe erzeugt XDTBox kein leeres XDT. Beim RT-3100 ist der Produktivlauf mit Rueckgabe und MEDISTAR-XDT-Ausgabe praktisch bestaetigt. RT-2100 und RT-5100 sind analog vorbereitet, brauchen aber noch eine eigene Live-Abnahme.
