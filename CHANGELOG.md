...
- CV-5000/CV-5000S-Zyklusnachlauf korrigiert: Ein erfolgreicher Rueckgabeexport erzeugt keinen zusaetzlichen Fehlerordner-Nachlauf `Bereits verarbeitet und nicht erneut exportiert` mehr. Identische AIS-/Geraetepaare bleiben gegen Doppelexport geschuetzt, neue Versionen von gleichnamigen AIS-Dateien wie `Patient.gdt` werden wieder als neuer Zyklus erkannt und koennen das Auswahlfenster ohne Reset erneut oeffnen.
- CV-5000/CV-5000S-Folgezyklus weiter stabilisiert: Nach Phase 1 merkt sich die App die vorhandenen Phoropter-Dateien als Baseline. Eine spaetere neue Rueckgabedatei startet den finalen Export auch dann, wenn ihr Dateizeitstempel aelter als die neue `Patient.gdt` ist; alte Baseline-Dateien bleiben dabei weiterhin ausgeschlossen.
- CV-5000/CV-5000S-direkter Folgezyklus mit gleicher Dateinamenstruktur korrigiert: Die Baseline-Pruefung nutzt fuer Phoropter-Rueckgabedateien eine erweiterte beobachtete Dateiversion, sodass ueberschriebene/geaenderte gleichnamige Rueckgabedateien den finalen Export starten, waehrend unveraenderte alte Baseline-Dateien weiter nicht mit neuer `Patient.gdt` gekoppelt werden.
