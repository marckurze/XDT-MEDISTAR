...
public sealed class MedistarTopconCt1PTemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-ct1p-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-ct1p-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-ct1p-default";

    private static readonly DateTimeOffset Timestamp = new(2026, 5, 22, 12, 0, 0, TimeSpan.Zero);
    ...
    [Fact]
    public void Export_ShouldContainOnlyCt1PAndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-ct1p-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON CT1P Templatepaket V1", importResult.Package.Metadata.Name);
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-ct1p-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-ct1p-v1.templatepackage.zip");
    }
}
