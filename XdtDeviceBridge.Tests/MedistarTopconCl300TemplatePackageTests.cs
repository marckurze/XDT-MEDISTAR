...
public sealed class MedistarTopconCl300TemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-cl300-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-cl300-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-cl300-default";
    ...
    [Fact]
    public void Export_ShouldContainOnlyCl300AndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-cl300-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON CL300 Templatepaket V1", importResult.Package.Metadata.Name);
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-cl300-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-cl300-v1.templatepackage.zip");
    }
}
