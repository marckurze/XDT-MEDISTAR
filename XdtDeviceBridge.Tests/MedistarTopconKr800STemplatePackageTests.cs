...
public sealed class MedistarTopconKr800STemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-kr800-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-kr800-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-kr800-default";
    ...
    [Fact]
    public void Export_ShouldContainOnlyKr800SAndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-kr-800s-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON KR-800S Templatepaket V1", importResult.Package.Metadata.Name);
        ...
        Assert.DoesNotContain(includedIds, id => id.Contains("trk", StringComparison.OrdinalIgnoreCase));
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-kr-800s-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-kr-800s-v1.templatepackage.zip");
    }
}
