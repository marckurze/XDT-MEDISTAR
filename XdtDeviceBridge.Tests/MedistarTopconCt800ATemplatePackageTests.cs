...
public sealed class MedistarTopconCt800ATemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-ct800a-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-ct800a-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-ct800a-default";
    ...
    [Fact]
    public void Export_ShouldContainOnlyCt800AAndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-ct-800a-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON CT-800A Templatepaket V1", importResult.Package.Metadata.Name);
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-ct-800a-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-ct800a-v1.templatepackage.zip");
    }
}
