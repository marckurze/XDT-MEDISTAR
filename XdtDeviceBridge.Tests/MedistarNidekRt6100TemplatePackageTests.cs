...
public sealed class MedistarNidekRt6100TemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-nidek-rt6100-default";
    private const string DefaultExportProfileId = "export-medistar-nidek-rt6100-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-nidek-rt6100-default";

    private static readonly DateTimeOffset Timestamp = new(2026, 5, 28, 12, 0, 0, TimeSpan.Zero);
    ...
    [Fact]
    public void Export_ShouldContainOnlyRt6100AndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-nidek-rt-6100-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + NIDEK RT-6100 Templatepaket V1", importResult.Package.Metadata.Name);
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-nidek-rt-6100-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-nidek-rt6100-v1.templatepackage.zip");
    }
}
