...
public sealed class MedistarTopconCv5000TemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-cv5000-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-cv5000-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-cv5000-default";

    private static readonly DateTimeOffset Timestamp = new(2026, 5, 23, 12, 0, 0, TimeSpan.Zero);
    ...
    [Fact]
    public void Export_ShouldContainOnlyCv5000AndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-cv-5000-cv-5000s-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON CV-5000 / CV-5000S Templatepaket V1", importResult.Package.Metadata.Name);
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-cv-5000-cv-5000s-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-cv5000-v1.templatepackage.zip");
    }
}
