...
public sealed class ProductiveUiSourceTests
{
    ...
    public void ProductiveNidekRtSerialWindow_ShouldExposeLiveDiagnosticsAndListenOnly()
    {
        ...
        Assert.DoesNotContain("Sendetest", floatingXaml);
        Assert.DoesNotContain("RS anfordern", floatingXaml);
        Assert.DoesNotContain("DTR-Toggle", floatingXaml);
        Assert.DoesNotContain("Direkt Writer-Frame senden", floatingXaml);
        Assert.DoesNotContain("RS + Writer ohne SD-Warten", floatingXaml);
        Assert.DoesNotContain("NidekRtFrameVariantComboBox", floatingXaml);
        Assert.DoesNotContain("CR nach EOT", floatingXaml);
        ...
        Assert.DoesNotContain("SerialRequestReadyRequested", floatingCode);
        Assert.DoesNotContain("SerialDirectWriterRequested", floatingCode);
        Assert.DoesNotContain("SerialRsWriterWithoutSdRequested", floatingCode);
        Assert.DoesNotContain("SelectedNidekRtSerialOutputFrameVariant", floatingCode);
        Assert.DoesNotContain("AppendCarriageReturnAfterEot", floatingCode);
        Assert.Contains("RunNidekRtSerialListenOnlyAsync", mainCode);
        Assert.Contains("RunNidekRtSerialProcessReturnAsync", mainCode);
        Assert.Contains("ResolveNidekRtSerialProductiveSendMode", mainCode);
