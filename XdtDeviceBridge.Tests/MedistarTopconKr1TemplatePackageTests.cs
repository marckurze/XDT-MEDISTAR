...
public sealed class MedistarTopconKr1TemplatePackageTests
{
    private const string DefaultAisProfileId = "ais-medistar-default";
    private const string DefaultDeviceProfileId = "device-topcon-kr1-default";
    private const string DefaultExportProfileId = "export-medistar-topcon-kr1-default";
    private const string DefaultInterfaceProfileId = "interface-medistar-topcon-kr1-default";
    ...
    [Fact]
    public void Export_ShouldContainOnlyKr1AndDependencies()
    {
        var importResult = _importer.Import(ExportTemplateCandidate());

        Assert.Equal("package-medistar-topcon-kr-1-v1", importResult.Package.Metadata.Id);
        Assert.Equal("MEDISTAR + TOPCON KR-1 Templatepaket V1", importResult.Package.Metadata.Name);
        ...
        Assert.DoesNotContain(includedIds, id => id.Contains("solos", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(includedIds, id => id.Contains("kr800", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(includedIds, id => id.Contains("trk2p", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(includedIds, id => id.Contains("ct1p", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(includedIds, id => id.Contains("ct800a", StringComparison.OrdinalIgnoreCase));
    ...
    private string ExportTemplateCandidate()
    {
        ...
        Assert.Equal("medistar-topcon-kr-1-v1.templatepackage.zip", result.SuggestedFileName);
    ...
    private static string CreateTempZipPath()
    {
        ...
        return Path.Combine(folder, "medistar-topcon-kr-1-v1.templatepackage.zip");
    }
}
