...
begin
  ...
  begin
    ...
    else if (Length(arr_PRISM) = 3) then
    begin
      if (T2WEndsStr('B', S1)) then
      begin
        S1:= arr_PRISM[0] + ' ' + 'B= ' + arr_PRISM[1];
      end
      else
      begin
        if (Trim(arr_PRISM[1]) = 'B') then
        begin
          S1:= arr_PRISM[0] + ' ' + 'B= ' + arr_PRISM[2];
        end;
      end;    
    end
    else if (Length(arr_PRISM) = 4) then   
    begin
...
var
  Data, Sep1, S1, ParsedData, R_S, L_S, R_C, L_C, R_Axis, L_Axis, R_Prism, R_Add, R_PD, L_Prism, R_Line, L_Line, L_Add, L_PD, PD, R_UV, L_UV: String;
  R_DataIndex, L_DataIndex, i: Integer;
  arrData1: TStringArray;
  bolAddSign, bolAddSignSeparator, bolAddAxisSeparator, bolUsePrismValues, bolAddPDValueToOutput: Boolean;
...
begin		
  ...
  // Verwende "True", damit die Prism Werte angefügt werden,
  // benutze "False", damit die Prism Werte nicht verwendet werden
  bolUsePrismValues:= True; 
    
  // Verwende "True", damit der PD Wert angefügt werden,
  // benutze "False", damit der PD Wert nicht verwendet werden
  ...
  begin
    ...
    // 
    if (R_DataIndex >= 0)
    and (T2WStartsStr(DATA_INDEX_PD,
                      S1))
    then
    begin
      R_PD := T2WStringReplace(S1,
                               DATA_INDEX_PD,
                               '',
                               False,
                               False);
      ...
      R_PD := Trim(R_PD);
      R_PD := FormatSignValue('PD=', R_PD, False, bolAddSignSeparator);
    ...
    begin
      ...
      L_PD := Trim(L_PD);
      L_PD := FormatSignValue('PD=', L_PD, False, bolAddSignSeparator);
  ...
  // Add right prism
  if (R_Prism <> '') and (bolUsePrismValues) then
  begin
    if (R_Line <> '') then
    begin
      R_Line := R_Line + ' ';
  ...
  // Add right pd
  if (R_PD <> '') and (bolAddPDValueToOutput) then
  begin
    if (R_Line <> '') then
    begin
      R_Line := R_Line + ' ';
  ...
  // Add left prism
  if (L_Prism <> '') and (bolUsePrismValues) then
  begin
    if (L_Line <> '') then
    begin
      L_Line := L_Line + ' ';
  ...
  // Add left pd
  if (L_PD <> '') and (bolAddPDValueToOutput) then
  begin
    if (L_Line <> '') then
    begin
      L_Line := L_Line + ' ';
