const
  ...
  PatientDataXMLTemplateFileName = 'EVR700-OPTO02-SEND-TEMPLATE-PATIENT.xml';
  PatientDataXMLTemplateEncoding = 0; // 0 = UTF8, 1 = UTF16
  PatientDataXMLExportFileName = 'NewImport_PatientData.xml';
  
  LensmeterXMLTemplateFileName = 'EVR700-OPTO02-SEND-TEMPLATE-LM.xml';
  LensmeterXMLTemplateEncoding = 0; // 0 = UTF8, 1 = UTF16
  LensmeterXMLExportFileName = 'NewImport_ALM.xml';
  
  RefraktometerObjektivXMLTemplateFileName = 'EVR700-OPTO02-SEND-TEMPLATE-AKR.xml';
  RefraktometerObjektivXMLTemplateEncoding = 0; // 0 = UTF8, 1 = UTF16
  RefraktometerObjektivXMLExportFileName = 'NewImport_AKR.xml';

  PhoropterXMLTemplateFileName = 'EVR700-OPTO02-SEND-TEMPLATE-SBJ.xml';
  PhoropterXMLTemplateEncoding = 0; // 0 = UTF8, 1 = UTF16
  PhoropterXMLExportFileName = 'NewImport_LastPrescription.xml';
...
var
  ...
  PatientID, PatientFirstName, PatientLastName, PatientDOB, LB, TemplateText: String;
...
begin
  ...
  // Verwende Dokumentation der Funktion "SysUtils.FormatDateTime"
  ...
  if (bolDisableSendingOfData) then
  begin
    Exit;
  end;
  ...
  DoExportPSGetGDTPatientData(PatientData);
  
  if (Length(PatientData) > 0) 
  and (Length(PatientData) > 3) then
  begin
    PatientFirstName:= PatientData[1];
    PatientLastName:= PatientData[0];
    PatientDOB:= PatientData[2];
    
    if (PatientDOB <> '') then
    begin
      // TODO:
			
			// S := T2WConvertDateTimeString(S, 'dd-mm-yyyy');
    end;
  end;
  ...
  // Open Patient Data XML template file and get result as string
  TemplateText:= '';
  LB:= '';
  
  DoPSGetXMLTemplateAsText(PatientDataXMLTemplateFileName, PatientDataXMLTemplateEncoding, LB, TemplateText);
  ...
  begin
    ...
    // Set patient data
    TemplateText:= T2WStringReplace(TemplateText, '<T2W_REPLACE/>', PatientFirstName, False, False);
    TemplateText:= T2WStringReplace(TemplateText, '<T2W_REPLACE/>', PatientLastName, False, False); 
    TemplateText:= T2WStringReplace(TemplateText, '<T2W_REPLACE/>', PatientDOB, False, False);    

    // Store XML template text as export file
    ExportFileName:= PatientDataXMLExportFileName;
        
    DoPSStoreXMLTemplateTextAsFile(ExportFileName, TemplateText, PatientDataXMLTemplateEncoding, LB);
  ...
  begin
    ...
    ExportFileName:= LensmeterXMLExportFileName;
  ...
  begin
    ...
    ExportFileName:= RefraktometerObjektivXMLExportFileName;
  ...
  begin
    ...
    ExportFileName:= PhoropterXMLExportFileName;
