const
  ...
  DATA_LINE_SEPARATOR_01 = #$0D#$0A; // #$0D -> CR / #$0A -> LF
...
begin
  ...
  // Die Einstellung ob Standard oder Extended kann im NIDEK RT-3100 Einstellungsmenü
  ...
  begin
    ...
    // 07-06-2023 FIX: Transmission problems occur if the lensmeter and autorefrometer values are entered
    // using the FM_REF-S form. These values are always written with a comma and could not be transmitted to the device.
    // If you manually replace the comma with a dot, the data will be transmitted correctly.
    // There is no adjustment option in the FM_REF-S form.
    // The glasses form model 8 also writes its values with a comma and not with a dot.
    // We have to replace the comma with a dot, before sending data to the device.
    ...
    begin
      ...
      begin
        ...
        // 07-06-2023 FIX: replace "," with "."
        if T2WContainsStr(S, ',') then
        begin    
          S:= T2WStringReplace(S, ',', '.', True, True);
        end;
      ...
      if (LM_R_S <> '') or (LM_R_Z <> '') or (LM_R_AX <> '') then
      begin
        LM_BLOCK:= 'DLM' +
                   DATA_SEPARATOR_STX
                   ;
      ...
      if (LM_L_S <> '') or (LM_L_Z <> '') or (LM_L_AX <> '') then
      begin
        if (LM_BLOCK = '') then
        begin
          LM_BLOCK:= 'DLM' +
    ...
    begin
      ...
      if (AR_R_S <> '') or (AR_R_Z <> '') or (AR_R_AX <> '') then
      begin
        AR_BLOCK:= 'DRM' +
                   DATA_SEPARATOR_STX
                   ;      
      ...
      if (AR_L_S <> '') or (AR_L_Z <> '') or (AR_L_AX <> '') then
      begin
        if (AR_BLOCK = '') then
        begin
          AR_BLOCK:= 'DRM' +
